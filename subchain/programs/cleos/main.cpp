...
eosio::chain_apis::read_only::get_info_results get_subchain_info() { //wangchao
   return call(url, get_subchain_info_func).as<eosio::chain_apis::read_only::get_info_results>();
}
...
std::string createsubchainAction(string& exthash)
{
   createsubchainRequest req;
   req.info.rpc_conf_file = "bitcoin.conf";
   req.info.datadir = "";

   req.subchainowner = "cf057bbfb72640471fd910bcb67639c22df9f92470936cddc1ade0e2f2e7dc4f";
   req.subchainid = "cf057bbfb72640471fd910bcb67639c22df9f92470936cddc1ade0e2f2e7dc4f";
   req.subchainname = "subchainname";
   req.subcoinname = "subcoinname";
   req.subchainseeds = "127.0.0.1:80|127.0.0.1:81";

   createsubchainResponse res;
   ...
   cout<<"extdatahash="<<res.extdatahash<<endl;
   cout<<"retDebugInfo="<<res.retDebugInfo<<endl;
   exthash = res.extdatahash;
   return res.retResult;
}

int main( int argc, char** argv ) {
std::string  exthash = "";
std::string result = createsubchainAction(exthash);
   ...
   //auto regCommand = app.add_subcommand("subchain", localized("Register subchain"));
   //regCommand->require_subcommand();
   get->add_subcommand("register", localized("Register subchain"))->set_callback([] {
      std::cout << fc::json::to_pretty_string(get_subchain_info()) << std::endl;
   });
  ...
  getBlock->add_option("number", numArg, localized("The number of the block to retrieve"))->required();
  getBlock->add_flag("--header-state", get_bhs, localized("Get block header state from fork database instead") );
  getBlock->set_callback([&blockArg,&numArg,&get_bhs] {
	 if( get_bhs ) {
		std::cout << fc::json::to_pretty_string(call(get_block_header_state_func, fc::mutable_variant_object("block_num_or_id", blockArg)("block_num", numArg))) << std::endl;
	 } else {
		std::cout << fc::json::to_pretty_string(call(get_block_func, fc::mutable_variant_object("block_num_or_id", blockArg)("block_num", numArg))) << std::endl;
	 }
  });
...
int read_subchain_info() {
   call(url, get_subchain_info_func).as<eosio::chain_apis::read_only::get_info_results>();
   cout<<"11111";
   return 1;
}
