...
//this function getregister the subchain to  --wangchao 2018.9.18
read_only::get_reg_results read_only::reg_subchain(const read_only::reg_subchain_params&) const {
   const auto& rm = db.get_resource_limits_manager();
   return {
      eosio::utilities::common::itoh(static_cast<uint32_t>(app().version())),
      db.get_chain_id(),
...
//this function get the subchain information --wangchao 2018.8.23
read_only::get_info_results read_only::get_subchain_info(const read_only::get_subchain_info_params&) const {
   const auto& rm = db.get_resource_limits_manager();
   return {
      eosio::utilities::common::itoh(static_cast<uint32_t>(app().version())),
      db.get_chain_id(),
...
//This is function for getting light-subchain infomation. --wangchao 2018.8.22 
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   signed_block_ptr block;
   signed_block_ptr block1[64];
   ...
   auto resp = fc::mutable_variant_object
          ("light_subchain_id", app().get_plugin<chain_plugin>().get_chain_id())
...
//This is function for getting light-subchain infomation. --wangchao 2018.8.22
fc::variant read_only::get_subchain_block(const read_only::get_subchain_block_params& params) const {
   vector<signed_block_ptr> block(64);
   static variant resp;
   mutable_variant_object mvo, mvo1;   

   EOS_ASSERT(!params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64, chain::block_id_type_exception, "Invalid Block number or ID, must be greater than 0 and less than 64 characters" );
   uint32_t start = atoi(params.block_num_or_id.c_str());
   uint32_t total = params.block_num;
   for (uint32_t i = 0; i < total; i++, start++) {
   string block_id = std::to_string(start);
   try {
      block[i] = db.fetch_block_by_id(fc::variant(block_id).as<block_id_type>());
      if (!block[i]) {
         block[i] = db.fetch_block_by_number(fc::to_uint64(block_id));
      }

   } EOS_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))

   EOS_ASSERT( block[i], unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));

   fc::variant pretty_output;
   abi_serializer::to_variant(*block[i], pretty_output, make_resolver(this));

   uint32_t ref_block_prefix = block[i]->id()._hash[1];
          mvo("light_subchain_id", app().get_plugin<chain_plugin>().get_chain_id());
          mvo(pretty_output.get_object());
          mvo("id", block[i]->id());
          mvo("block_num",block[i]->block_num());
	  mvo1(block_id.c_str(), mvo);

   	//resp = fc::mutable_variant_object
	//				(block_id.c_str(), fc::variants({
	//				   fc::mutable_variant_object()
	//					  ("light_subchain_id", app().get_plugin<chain_plugin>().get_chain_id())
	//					  (pretty_output.get_object())
	//					  ("id", block[i]->id())
	//					  ("block_num",block[i]->block_num())
	//				   })
	//			   );
   }
   return mvo1;
