...
void Shutdown()
{
	...
	bridgeownerpwallet->Flush(false);
    }
#endif
    MapPort(false);
...

    {
        ...
        delete pblocktree;
        delete psubblocktree;
        pblocktree = nullptr;
        psubblocktree = nullptr;

        delete psubchainmeta;
        psubchainmeta = nullptr;
    }
#ifdef ENABLE_WALLET
    for (CWalletRef pwallet : vpwallets) {
        pwallet->Flush(true);
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    LogPrintf("hsss2\n");
        ...
        while (true) {
            CDiskBlockPos pos(nFile, 0);
            if (!fs::exists(GetBlockPosFilename(pos, "subblk")))
                break; // No block files left to reindex
            FILE *file = OpenSubBlockFile(pos, true);
            if (!file)
                break; // This error is logged in OpenBlockFile
            LogPrintf("Reindexing subblock file subblk%05u.dat...\n", (unsigned int)nFile);
            LoadExternalSubBlockFile(chainparams, file, &pos);
            nFile++;
        }

        subchainmempool.LoadSubChainMetaData();

        pblocktree->WriteReindexing(false);
        psubblocktree->WriteReindexing(false);
        fReindex = false;
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                psubblocktree = new CBlockTreeDB(nBlockTreeDBCache, false, fReset,"subblocks");

                psubchainmeta = new CSubChainMetaDB(nBlockTreeDBCache, false, fReset);

                if (fReset) {
                    pblocktree->WriteReindexing(true);
                    psubblocktree->WriteReindexing(true);
                ...
                if (!LoadSubBlockIndex(chainparams)) {
                    strLoadError = _("Error loading subblock database");
                    break;
                }
    ...
    if (!CWallet::InitLoadWallet())
        return false;
    if (!CWallet::InitLoadOwnerWallet())
        return false;
#else
