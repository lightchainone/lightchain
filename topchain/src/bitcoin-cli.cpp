...
int CommandLineRPC(int argc, char *argv[])
{
    ...
    if (strPrint != "") {
        fprintf((nRet == 0 ? stdout : stderr), "strprint:%s\n", strPrint.c_str());
    }
    if(strcmp(argv[1] ,"createsubchaininfo") == 0){
            const char* input[] = {"bitcoin-cli","commitsubchaininfo",strPrint.c_str()};
	    fprintf(stdout,"begin send first %s commit %s\n",argv[1],input[1]);
	    CommandLineRPC(3, const_cast<char**>(input));
	    fprintf(stdout,"end send commit %s\n",input[1]);
    }
    return nRet;
}
