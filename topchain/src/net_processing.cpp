...
static CCriticalSection cs_most_recent_subblock;
static std::shared_ptr<const CSubBlock> most_recent_subblock;
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    case MSG_SUBBLOCK:
        LogPrintf("testxx1");
        return mapSubBlockIndex.count(inv.hash);
    }
...
void RelaySubBlock(const CSubBlock& block, CConnman* connman)
{
    CInv inv(MSG_SUBBLOCK, block.GetHash());
    connman->ForEachNode([&inv](CNode* pnode)
    {
        pnode->PushInventory(inv);
    });
...
void static ProcessGetData(CNode* pfrom, const Consensus::Params& consensusParams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            {
		    LogPrint(BCLog::NET, "recv getdata subblock (hash=%s) peer=%d\n",   inv.hash.ToString(), pfrom->GetId());
                bool send = false;
                std::shared_ptr<const CSubBlock> a_recent_subblock;
                {
                    LOCK(cs_most_recent_subblock);
                    a_recent_subblock = most_recent_subblock;
                }

		extern SubBlockMap mapSubBlockIndex;
                SubBlockMap::iterator mi = mapSubBlockIndex.find(inv.hash);
		...
		if (send)
		{
			std::shared_ptr<const CSubBlock> pblock;
			//TODO open it 
			if (a_recent_subblock && a_recent_subblock->GetHash() == (*mi).second->GetBlockHash()) {
				pblock = a_recent_subblock;
			} else {
				// Send block from disk
				CSubBlock block;
				std::shared_ptr<CSubBlock> pblockRead = std::make_shared<CSubBlock>();
				if (!ReadSubBlockFromDisk(*pblockRead, (*mi).second, consensusParams))
					assert(!"cannot load block from disk");
				pblock = pblockRead;
			}
			connman->PushMessage(pfrom, msgMaker.Make(SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::SUBBLOCK, *pblock));
                    {
			    LOCK(cs_most_recent_subblock);
			    most_recent_subblock = pblock;
                    } 
		}
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::REJECT)
    {
        if (LogAcceptCategory(BCLog::NET)) {
            try {
                ...
                if (strMsg == NetMsgType::BLOCK || strMsg == NetMsgType::SUBBLOCK || strMsg == NetMsgType::TX)
                {
                    uint256 hash;
                    vRecv >> hash;
    ...
    else if (strCommand == NetMsgType::SUBBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        std::shared_ptr<CSubBlock> pblock = std::make_shared<CSubBlock>();
        vRecv >> *pblock;

        LogPrint(BCLog::NET, "received subblock %s peer=%d\n", pblock->GetHash().ToString(), pfrom->GetId());
        ...
        bool fNewBlock = false;
//TODO write it
        ProcessNewSubBlock(chainparams, pblock, forceProcessing, &fNewBlock);
        if (fNewBlock) {
            pfrom->nLastBlockTime = GetTime();
        } else {
            LOCK(cs_main);
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        std::vector<CInv> vInv;
        {
            ...
            vInv.reserve(std::max<size_t>(pto->vInventorySubBlockToSend.size(), INVENTORY_BROADCAST_MAX));

            // Add blocks
            for (const uint256& hash : pto->vInventorySubBlockToSend) {
                vInv.push_back(CInv(MSG_SUBBLOCK, hash));
 LogPrintf("\naddinv hash vInv=%d s=%s\n",vInv.size(), vInv[0].ToString());
                if (vInv.size() == MAX_INV_SZ) {
                    connman->PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv));
                    vInv.clear();
                }
            }
            pto->vInventorySubBlockToSend.clear();
        ...
        while (!pto->mapAskFor.empty() && (*pto->mapAskFor.begin()).first <= nNow)
        {
            ...
            if (!AlreadyHave(inv))
            {
                ...
                LogPrint(BCLog::NET, "alreadyexists %s peer=%d\n", inv.ToString(), pto->GetId());
