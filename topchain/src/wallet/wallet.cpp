...
CWalletRef bridgeownerpwallet;
CKeyID bridgeownerkeyid;
...
const char * DEFAULT_OWNERWALLET_DAT = ".bridge.owner.dat";
...
LogPrintf("checkhere4\n");
        SyncTransaction(ptx);
    }
    for (size_t i = 0; i < pblock->vtx.size(); i++) {
LogPrintf("checkhere5\n");
...
bool CWallet::CommitTransaction(CWalletTx& wtxNew, CReserveKey& reservekey, CConnman* connman, CValidationState& state)
{
    {
        LOCK2(cs_main, cs_wallet);
        LogPrintf("CommitTransaction:%s re=%s\n", wtxNew.tx->ToString(),reservekey.toString());
        {
        LogPrintf("h1");
...
{
    LogPrintf("xxxxhaxx keypool_id=%d\n", keypool_id);
    AssertLockHeld(cs_wallet);
    LogPrintf("xxxxhbxx\n");
    bool internal = setInternalKeyPool.count(keypool_id);
    for(auto iter=setInternalKeyPool.begin(); iter!=setInternalKeyPool.end(); ++iter)
    {
         LogPrintf("interanlkey:%ld\n",*iter);
    }

    for(auto iter=setExternalKeyPool.begin(); iter!=setExternalKeyPool.end(); ++iter)
    {
         LogPrintf("Exteranlkey:%ld\n",*iter);
    }
    LogPrintf("xxxxhcxx\n");
...
bool CWallet::InitLoadWallet()
{
    ...
    for (const std::string& walletFile : gArgs.GetArgs("-wallet")) {
        if(walletFile == DEFAULT_OWNERWALLET_DAT){
             return false;
        }
...
bool CWallet::InitLoadOwnerWallet()
{
    if (gArgs.GetBoolArg("-disablebridgeowner", DEFAULT_DISABLE_OWNER)) {
        LogPrintf("owner disabled!\n");
        return true;
    }
    const std::string& walletFile = gArgs.GetArg("-bridgeowner", DEFAULT_OWNERWALLET_DAT); 
    const std::vector<std::string> vwallet =  gArgs.GetArgs("-wallet");
    std::vector<std::string>::const_iterator iter = find(vwallet.begin(), vwallet.end(), walletFile);
    if(iter != vwallet.end() ||  walletFile == DEFAULT_WALLET_DAT){
        LogPrintf("invalid BRIDAGE OWNER!\n");
        return false;
    }
