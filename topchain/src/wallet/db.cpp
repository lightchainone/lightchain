...
LogPrintf("xxxx mock=%d create=%d nFlag=%d \n",fMockDb, fCreate, nFlags);
            if (fMockDb) {
            ...
            if (ret != 0) {
                throw std::runtime_error(strprintf("CDB: Error %d, can't open database %s %d %d", ret, strFilename, fCreate,fMockDb));
            }
