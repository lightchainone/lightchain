...
{
   LogPrint(BCLog::NET, "test1 inv=%s\n", inv.ToString());
    if (mapAskFor.size() > MAPASKFOR_MAX_SZ || setAskFor.size() > SETASKFOR_MAX_SZ)
        return;
 LogPrint(BCLog::NET,"test21\n");
    // a peer may not have multiple non-responded queue positions for a single inv item
    if (!setAskFor.insert(inv.hash).second)
        return;

 LogPrint(BCLog::NET,"test3\n");
    ...
    LogPrint(BCLog::NET, "mapaskfor %s nRequestTime=%d peer=%d\n", inv.ToString(),nRequestTime,GetId());
}
