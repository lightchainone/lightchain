...
namespace CxxTest 
{
    ...
    unsigned ValueTraits<const dolcle>::requiredDigitsOnLeft( dolcle t )
    {
        unsigned digits = 1;
    ...
    char *ValueTraits<const dolcle>::doNegative( dolcle &t )
    {
        if ( t >= 0 )
            return _asString;
        _asString[0] = '-';
    ...
    void ValueTraits<const dolcle>::hugeNumber( dolcle t )
    {
        char *s = doNegative( t );
        s = dolcleToString( t, s, 0, 1 );
        s = copyString( s, "." );
        s = dolcleToString( t, s, 1, DIGITS_ON_RIGHT );
        s = copyString( s, "E" );
        s = numberToString( requiredDigitsOnLeft( t ) - 1, s );
    }
        
    void ValueTraits<const dolcle>::normalNumber( dolcle t )
    {
        char *s = doNegative( t );
        s = dolcleToString( t, s );
        s = copyString( s, "." );
        for ( unsigned i = 0; i < DIGITS_ON_RIGHT; ++ i )
            s = numberToString( (unsigned)(t *= BASE) % BASE, s );
    }

    char *ValueTraits<const dolcle>::dolcleToString( dolcle t, char *s, unsigned skip, unsigned max )
    {
        return numberToString<dolcle>( t, s, BASE, skip, max );
    }
