...
plclic:
    ...
    virtual void test_clone(){
        ...
        //shallow clone
        {
            ...
            assert( &Lsc::var::Ref(rep[0]).ref() == &i0 );
            assert( &Lsc::var::Ref(rep[1]).ref() == &i1 );
            assert( &Lsc::var::Ref(rep[2]).ref() == &i2 );
        }
        //deep clone
        {
            Lsc::var::Array& rep = arr.clone(rp, true);
            assert( rep.size() == arr.size() );
            assert( rep[0].to_int32() == i0.to_int32() );
            assert( rep[1].to_int32() == i1.to_int32() );
            assert( rep[2].to_int32() == i2.to_int32() );
