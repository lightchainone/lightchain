...
#include<cassert>
/*
void test___Lsc::NgAllocImpl(){
    cout<<endl<<"enter "<<__func__<<"():"<<endl;

    __Lsc::NgAllocImpl impl10(10);// a multiple of sizeof(size_t) is better
    impl10.allocate(10);
    impl10.allocate(0);
    impl10.allocate(9);
    impl10.allocate(8);
    ...
    typedef __Lsc::NgAllocImpl::config_t conf_t;
    conf_t config; //取默认值
    ...
    __Lsc::NgAllocImpl impl_conf(config);
    //todo: 增加对__Lsc::NgAllocImpl(const __Lsc::NgAllocImpl::config_t&)生成的对象的测试
...
void test_construct_and_destroy(){
    ...
    char buf2[sizeof(C)];
    void *q = buf2;
    Lsc::construct<C,C>(q,p);
    int aa = 4;
    Lsc::construct<C,int>(q,&aa); //call C::C(int)
...
void test_attach(){
...

    {
        ...
        //atach an object;
        C *pc = new C;
        rp.attach( pc, Lsc::Lsc_delete<C> );

        C *pc_arr = new C[2];
        rp.attach( pc_arr, Lsc::Lsc_delete_array<C> );
...
void test_allocator(){
    ...
    // 编译测试
    vector<int,Lsc::NgAlloc<int> > v;
    map<dolcle, char*, Lsc::NgAlloc<dolcle> >m;
    set<char, Lsc::NgAlloc<char> > s;
    list<int, Lsc::NgAlloc<int> > l;
    deque<int, Lsc::NgAlloc<int> > di;
    deque<vector<int, Lsc::NgAlloc<int> >, Lsc::NgAlloc<vector<int, Lsc::NgAlloc<int> > > > dv;
...
void test_with_mempool(){
...

    {
        Lsc::debugpool pool;
        Lsc::ResourcePool rp(pool);
        Lsc::pool_allocator<char> alloc = rp.get_allocator();
        char *p = alloc.allocate(2);
        alloc.deallocate(p, 2);
        Lsc::pool_allocator<C> alloc2 = rp.get_allocator();
...

    {
        ...
        Lsc::ResourcePool rp(pool);
        Lsc::var::String str = 
            rp.create<Lsc::var::String>(Lsc::var::String::allocator_type( &rp.get_mempool() ));
        str = "hello word";
        printf("%s\n",str.c_str());
    }    
}

class EException: plclic Lsc::BasicException<EException, Lsc::Exception>{};
...
class BufPool: plclic Lsc::mempool{
plclic:
    ...
    virtual void * malloc(size_t size){
        if ( _count + size > _size ){
            throw Lsc::BadAllocException()<<BSL_EARG<<"need:"<<size<<" left:"<<_size - _count;
        }
    ...
    virtual void free( void *p, size_t len ){
        ...
        if ( iter == _map.end() || p != iter->first || len != iter->second ){
            throw Lsc::BadArgumentException()<<BSL_EARG<<"p:"<<p<<" len:"<<len;
        }
        //do nothing
    }
private:
...
void test_except(){
...

    {
        ...
        Lsc::ResourcePool rp(throw_pool);
        
        ASSERT_THROW( rp.create<int>(), Lsc::BadAllocException );
        ASSERT_THROW( rp.create<C>(123), Lsc::BadAllocException );
        assert( NULL == rp.createp<int>() );
        assert( NULL == rp.createp<C>(123) );

        C *pc = new C();
        ASSERT_THROW( rp.attach( pc, Lsc::Lsc_delete<C> ), Lsc::BadAllocException );
    }
}
