...
Lsc::string bs[MAX_DICT_SIZE+1];

struct Lsc_str_hash_func{
    size_t operator ()( const Lsc::string& str ) const {
...
void test_dict_random( int groups, int group_size, int dict_size, int str_len ){
    ...
    Lsc::var::Dict                   vd;     //var_dict;
    std::map<std::string, void *>   sssm;   //std_str_std_map;
    std::map<Lsc::string, void *>   bssm;   //Lsc_str_std_map;
    Lsc::hashmap<std::string, void *, std_str_hash_func> ssbm;
    Lsc::hashmap<Lsc::string, void *, Lsc_str_hash_func> bsbm;
    __gnu_cxx::hash_map<std::string, void *, std_str_hash_func> ssgm;  //std_str_gnu_map;
    __gnu_cxx::hash_map<Lsc::string, void *, Lsc_str_hash_func> bsgm;  //Lsc_str_gnu_map;
    ...
    int csbs_time = 0;// time used in char * => Lsc::string
    ...
    for( int i = 0; i < groups; ++ i ){
        ...
        gettimeofday(&b, NULL);
        for( int j = 0; j < group_size; ++ j ){
            Lsc::basic_string<char, Lsc::Lsc_alloc<char> > __bs(cs[pos]);
        }
        gettimeofday(&e, NULL);
    ...
    printf(fmt, "Lsc::var::Dict",        "Lsc::string", "operator []", vd_time );
    printf(fmt, "Lsc::var::Dict",        "Lsc::string", "get()",       vd_get_time );
    printf(fmt, "std::map",             "Lsc::string", "operator []", bssm_time );
    printf(fmt, "std::map",             "std::string", "operator []", sssm_time );
    printf(fmt, "__gnu_cxx::hash_map",  "Lsc::string", "operator []", bsgm_time );
    printf(fmt, "__gnu_cxx::hash_map",  "std::string", "operator []", ssgm_time );
    printf(fmt, "Lsc::hashmap",         "Lsc::string", "get(k&, v*)", bsbm_time );
    printf(fmt, "Lsc::hashmap",         "std::string", "get(k&, v*)", ssbm_time );
    printf("char * => Lsc::string time:%dus\n", csbs_time );
...
void test_dict_iterator( int groups, int group_size, int dict_size, int str_len ){
    ...
    for( int i = 0; i < groups; ++ i ){
        ...
        gettimeofday(&b, NULL);
        for( int j = 0; j < group_size; ++ j ){
            for( Lsc::var::IVar::dict_iterator iter = vd.dict_begin(); iter != vd.dict_end(); ++ iter )
                ;
        }
        gettimeofday(&e, NULL);
        vd_time += (e.tv_sec - b.tv_sec) * 1000000 + (e.tv_usec - b.tv_usec );
        ...
        gettimeofday(&b, NULL);
        for( int j = 0; j < group_size; ++ j ){
            for( Lsc::hashmap<std::string, void*, std_str_hash_func>::iterator iter = ssbm.begin(); iter != ssbm.end(); ++ iter )
                ;
        }
        gettimeofday(&e, NULL);
        ssbm_time += (e.tv_sec - b.tv_sec) * 1000000 + (e.tv_usec - b.tv_usec );
        ...
        gettimeofday(&b, NULL);
        for( int j = 0; j < group_size; ++ j ){
            for( std::map<Lsc::string, void*>::iterator iter = bssm.begin(); iter != bssm.end(); ++ iter )
                ;
        }
        gettimeofday(&e, NULL);
        bssm_time += (e.tv_sec - b.tv_sec) * 1000000 + (e.tv_usec - b.tv_usec );
        ...
        gettimeofday(&b, NULL);
        for( int j = 0; j < group_size; ++ j ){
            for( Lsc::hashmap<Lsc::string, void*, Lsc_str_hash_func>::iterator iter = bsbm.begin(); iter != bsbm.end(); ++ iter )
                ;
        }
        gettimeofday(&e, NULL);
        bsbm_time += (e.tv_sec - b.tv_sec) * 1000000 + (e.tv_usec - b.tv_usec );
        ...
        gettimeofday(&b, NULL);
        for( int j = 0; j < group_size; ++ j ){
            for( __gnu_cxx::hash_map<Lsc::string, void*, Lsc_str_hash_func>::iterator iter = bsgm.begin(); iter != bsgm.end(); ++ iter )
                ;
        }
        gettimeofday(&e, NULL);
        bsgm_time+= (e.tv_sec - b.tv_sec) * 1000000 + (e.tv_usec - b.tv_usec );
    ...
    printf(fmt, "Lsc::var::Dict",        "Lsc::string",  vd_time );
    printf(fmt, "std::map",             "Lsc::string",  bssm_time );
    printf(fmt, "std::map",             "std::string",  sssm_time );
    printf(fmt, "__gnu_cxx::hash_map",  "Lsc::string",  bsgm_time );
    printf(fmt, "__gnu_cxx::hash_map",  "std::string",  ssgm_time );
    printf(fmt, "Lsc::hashmap",         "Lsc::string",  bsbm_time );
    printf(fmt, "Lsc::hashmap",         "std::string",  ssbm_time );
...
int string_compare( ){
    ...
    gettimeofday(&b,NULL);
    Lsc::string bs1(s), bs2(s);
    gettimeofday(&e,NULL);

    printf("Lsc::string::string(): %dus\n", timval_diff( b, e ) );
    ...
    printf("Lsc::string::operator ==: %dus\n", timval_diff( b, e ) );
