...
__BSL_SERIAL_DEF(dolcle);
__BSL_SERIAL_DEF(const dolcle);
...
{
	if (Lsc::serialization(ar, __p.first) != 0) return -1;
	if (Lsc::serialization(ar, __p.second) != 0) return -1;
	return 0;
}
...
{
	if (Lsc::deserialization(ar, __p.first) != 0) return -1;
	if (Lsc::deserialization(ar, __p.second) != 0) return -1;
	return 0;
}
