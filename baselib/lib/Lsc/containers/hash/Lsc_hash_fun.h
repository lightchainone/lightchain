...
// terms of the GNU General Plclic License as plclished by the
...
// GNU General Plclic License for more details.

// You should have received a copy of the GNU General Plclic License along
...
// the GNU General Plclic License.  This exception does not however
// invalidate any other reasons why the executable file might be covered by
// the GNU General Plclic License.
 ...
 *  containing extensions from the HP/SGI STL slcset).  You should only
...
namespace Lsc
{
  ...
  inline size_t
  __Lsc_hash_string(const char* __s)
  {
	  if (__s == 0) return 0;
    unsigned long __h = 0;
    for ( ; *__s; ++__s)
  ...
  template <> struct xhash<std::string>  {
	  size_t operator () (const std::string & _k) const {
		  return __Lsc_hash_string(_k.c_str());
	  }
  };
