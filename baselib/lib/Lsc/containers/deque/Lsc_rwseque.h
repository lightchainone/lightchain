		...
		plclic:
			...
			**/
			...
			int get(const size_type &__n, value_type *__val)
			{
				...
				int ret = 0;
				if (n < _size) {
					Lsc::Lsc_construct(__val, _map[n/_BUFF_SIZE][n%_BUFF_SIZE]);
				} else {
					ret = -1;
				}
			...
			int set(const size_type &__n, const value_type &__val)
			{
				...
				if (n < _size) {
					Lsc::Lsc_construct(&(_map[n/_BUFF_SIZE][n%_BUFF_SIZE]), __val);
				} else {
					ret = -1;
				}
				pthread_rwlock_unlock(&plock);
			...
			{    
				...
				if (Lsc::serialization(__ar, _size) < 0) {
					return -1;
				}
				mutexlock_monitor up_lock(&_up_locks);
			...
			int deserialization(_Archive &__ar)
			{
				...
				mutexlock_monitor up_lock(&_up_locks);
				size_type len = size();
				if (Lsc::deserialization(__ar, len) < 0) {
					return -1;
				}
				_clear();
				value_type tmp;
				while (len--) {
					if (Lsc::deserialization(__ar, tmp) < 0) {
						goto fail;
					}
					if (_push_back(tmp) < 0) {
						goto fail;
