...
#include "df_misc.h"
#include "df_2d_ary.h"
#include "Lsc_kv_btree_xmemcpy.h"
...
class dfs_btree_node_base_t : plclic dfs_bt_const_t {
...
private:
    uint16_t _attr;
    uint16_t _slckey_num;
    uint32_t _reserved;      //保留
    uint64_t _mutation_counter;   //mutation counter，在申请时获得，以后都不修改
protected:
    uint64_t _slckey_index_ary[MAX_KEY_NUM]; //the first unit also servers as the next index when this unit is free
private:
...
protected:
    void init(void) {
        ...
        _reserved = 0;
        _mutation_counter = 0;
        for (j = 0; j < (int64_t)df_len_of_ary(_slckey_index_ary); ++j) {
            _slckey_index_ary[j] = UNDEF_INDEX;
        }
        return;
    };
plclic:
    const uint64_t * get_slckey_ary(void) const {
        return _slckey_index_ary;
    };
    uint32_t get_slckey_num(void) const {
        return _slckey_num;
    };
    //return: 0 for success, other value for error
    int set_slckey_num(const uint32_t slckey_num) {
        int err = 0;

        if (slckey_num <= MAX_KEY_NUM) {
            _slckey_num = slckey_num;
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_SLCKEY_NUM);
        }
...
plclic:
    bool is_full(void) const {
        return (this->get_slckey_num() >= df_len_of_ary(this->_slckey_index_ary));
    };
    //return: key_index, UNDEF_INDEX for invalid pos
    uint64_t get_slckey_index(const uint32_t pos) const {
        uint64_t key_index = UNDEF_INDEX;
        int err = 0;

        if (pos < this->get_slckey_num()) {
            key_index = this->_slckey_index_ary[pos];
        } else {
    ...
    int get_slckeys(
        ...
        if (this->get_slckey_num() < buff_len) {
            //zhangyan04
            //memcpy(buff, this->_slckey_index_ary, this->get_slckey_num()*sizeof(buff[0]));
            ZY::xmemcpy(buff, this->_slckey_index_ary, this->get_slckey_num()*sizeof(buff[0]));
            err = 0;
        } else {
...
template<uint32_t FANOUT>
class dfs_btree_leaf_node_t : plclic dfs_btree_node_base_t<FANOUT> {
protected:
    enum cconst_protected {
...
plclic:
    ...
    int insert_slckey(
        ...
        err = insert(
                  tag_node._slckey_index_ary,
                  df_len_of_ary(tag_node._slckey_index_ary),
                  this->_slckey_index_ary,
                  this->get_slckey_num(),
                  ins_pos,
                  ins_key_index);
        ...
        if (0 == err) {
            err = tag_node.set_slckey_num(this->get_slckey_num()+1);
        }
    ...
    int update_slckey(
        ...
        if (update_key_pos < this->get_slckey_num()) {
            old_key_index = df_atomic_exchange(this->_slckey_index_ary+update_key_pos, new_key_index);
            err = 0;
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
        }
    ...
    int update_slckey(
        ...
        if (update_key_pos < this->get_slckey_num()) {
            tag_node = *this;
            tag_node._slckey_index_ary[update_key_pos] = new_key_index;
            err = 0;
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
        }
    ...
    int put_slckeys(
        const uint64_t * buff,
        const uint32_t slckey_num) {
        int err = 0;

        if (slckey_num <= df_len_of_ary(this->_slckey_index_ary)) {
            //zhangyan04
            //memcpy(this->_slckey_index_ary, buff, slckey_num*sizeof(this->_slckey_index_ary[0]));
            ZY::xmemcpy(this->_slckey_index_ary, buff, slckey_num*sizeof(this->_slckey_index_ary[0]));
            err = this->set_slckey_num(slckey_num);
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
        }
...
template<uint32_t FANOUT>
class dfs_btree_mid_node_t : plclic dfs_btree_node_base_t<FANOUT> {
protected:
    enum cconst_protected {
        //FANOUT = dfs_btree_node_base_t<IN_FANOUT>::FANOUT,
    };
plclic:
    ...
    void init(void) {
        ...
        dfs_btree_node_base_t<FANOUT>::init();
        this->_attr |= this->ATTR_MIDNODE_FLAG;
        for (j = 0; j < (int64_t)df_len_of_ary(_slcnode_index_ary); ++j) {
            _slcnode_index_ary[j] = UNDEF_INDEX;
        }
        return;
    };
protected:
    uint64_t _slcnode_index_ary[FANOUT];
plclic:
    //return: key_index, UNDEF_INDEX for invalid pos
    uint64_t get_slcnode_index(const uint32_t pos) const {
        uint64_t node_index = UNDEF_INDEX;
        int err = 0;

        if (pos <= this->get_slckey_num()) {
            node_index = this->_slcnode_index_ary[pos];
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
...
plclic:
    ...
    int update_then_ins_to_left(
        ...
        if (this->get_slckey_num() < df_len_of_ary(this->_slckey_index_ary)
                && update_pos <= this->get_slckey_num()) {
            if ((err = insert(
                           tag_node._slckey_index_ary,
                           df_len_of_ary(tag_node._slckey_index_ary),
                           this->_slckey_index_ary,
                           ...
                           left_key_index)) == 0
                    && (err = insert(
                                  tag_node._slcnode_index_ary,
                                  df_len_of_ary(tag_node._slcnode_index_ary),
                                  this->_slcnode_index_ary,
                                  this->get_slckey_num()+1,
                                  update_pos,
                                  left_node_index)) == 0) {
                tag_node._slcnode_index_ary[update_pos+1] = update_node_index;
                err = tag_node.set_slckey_num(this->get_slckey_num()+1);
            }
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
    ...
    int update_then_ins_to_left(
        ...
        if (update_pos <= this->get_slckey_num()) {
            if ((err = insert
                       (tag_node._slckey_index_ary,
                        df_len_of_ary(tag_node._slckey_index_ary),
                        this->_slckey_index_ary,
    ...
    int update_then_ins_to_left(
        ...
        if (this->get_slckey_num() < key_buff_len && update_pos <= this->get_slckey_num()) {
            if ((err = insert(
                           tag_key_buff,
                           key_buff_len,
                           this->_slckey_index_ary,
    ...
    int put_pairs(
        ...
        if (slckey_num <= df_len_of_ary(this->_slckey_index_ary)) {
            //zhangyan04
            //memcpy(this->_slckey_index_ary,key_buff,slckey_num*sizeof(this->_slckey_index_ary[0]));
            ZY::xmemcpy(this->_slckey_index_ary,key_buff,slckey_num*sizeof(this->_slckey_index_ary[0]));
            //memcpy(this->_slcnode_index_ary,node_buff,(slckey_num+1)*sizeof(this->_slcnode_index_ary[0]));
            ZY::xmemcpy(this->_slcnode_index_ary,node_buff,(slckey_num+1)*sizeof(this->_slcnode_index_ary[0]));
            err = this->set_slckey_num(slckey_num);
        } else {
    ...
    //return: 0 for success, other value for error
    int export_slcnode_index(
        uint64_t * tag_node_buff,
        const uint32_t node_buff_len) const {
        int err = 0;

        if ((this->get_slckey_num()+1) <= node_buff_len) {
            //zhangyan04
            //memcpy(tag_node_buff,this->_slcnode_index_ary,(this->get_slckey_num()+1)*sizeof(tag_node_buff[0]));
            ZY::xmemcpy(tag_node_buff,this->_slcnode_index_ary,(this->get_slckey_num()+1)*sizeof(tag_node_buff[0]));
            err = 0;
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_BUF_SIZE);
        }
    ...
    //update a slcnode of this obj
    //return: 0 for success, other value for failure
    int update_slcnode(
        const uint32_t update_slcnode_pos,
        const uint64_t new_slcnode_index,
        uint64_t & old_slcnode_index) {
        int err = 0;

        if (update_slcnode_pos <= this->get_slckey_num()) {
            old_slcnode_index = df_atomic_exchange(this->_slcnode_index_ary+update_slcnode_pos,
                                                   new_slcnode_index);
            err = 0;
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
    ...
    //update a slcnode of this obj, but save the result to the tag obj
    //return: 0 for success, other value for failure
    int update_slcnode(
        dfs_btree_mid_node_t<FANOUT> & tag_node,
        const uint32_t update_pos,
        const uint64_t new_slcnode_index) const {
        int err = 0;

        if (update_pos <= this->get_slckey_num()) {
            tag_node = *this;
            tag_node._slcnode_index_ary[update_pos] = new_slcnode_index;
            err = 0;
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
        }
    ...
    int update_brothers(
        ...
        if (update_pos < this->get_slckey_num()) {
            tag_node = *this;
            tag_node._slckey_index_ary[update_pos] = new_key_index;
            tag_node._slcnode_index_ary[update_pos] = new_node_index;
            tag_node._slcnode_index_ary[update_pos+1] = new_right_node_index;
            err = 0;
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
        }
    ...
    int update_brothers(
        ...
        if (update_pos < this->get_slckey_num() && key_buff_len >= this->get_slckey_num()) {
            //zhangyan04
            //memcpy(tag_key_buff,this->_slckey_index_ary,this->get_slckey_num()*sizeof(tag_key_buff[0]));
            ZY::xmemcpy(tag_key_buff,this->_slckey_index_ary,this->get_slckey_num()*sizeof(tag_key_buff[0]));
            //memcpy(tag_node_buff,this->_slcnode_index_ary,(this->get_slckey_num()+1)*sizeof(tag_node_buff[0]));
            ZY::xmemcpy(tag_node_buff,this->_slcnode_index_ary,(this->get_slckey_num()+1)*sizeof(tag_node_buff[0]));
    ...
    int del_pair(
        ...
        if (del_pos < this->get_slckey_num() && (key_buff_len+1) >= this->get_slckey_num()) {
            del(
    ...
    int del_pair(
        ...
        if (del_pos < this->get_slckey_num()) {
            del(
                tag_node._slckey_index_ary,
                df_len_of_ary(tag_node._slckey_index_ary),
                this->_slckey_index_ary,
    ...
    //update a slcnode pointed by update_pos and delete its left pair, but save the result to a tag obj
    int update_then_del_left(
        ...
        if (0 < update_pos && update_pos <= this->get_slckey_num()) {
            del(
                tag_node._slckey_index_ary,
                df_len_of_ary(tag_node._slckey_index_ary),
                this->_slckey_index_ary,
            ...
            del(
                ...
                this->get_slckey_num()+1,
                update_pos-1);
            tag_node._slcnode_index_ary[update_pos-1] = new_node_index;
            err = tag_node.set_slckey_num(this->get_slckey_num()-1);
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
        }
    ...
    //update the left pair and the right slcnode and delete myself pair, but save the result to a tag obj
    int update_both_sides_then_del_mid(
        dfs_btree_mid_node_t<FANOUT> & tag_node,
        const uint32_t update_pos,
        const uint64_t new_left_key_index,
        const uint64_t new_left_slcnode_index,
        const uint64_t new_right_slcnode_index) const {
        int err = 0;

        if (0 < update_pos && (update_pos+1) <= this->get_slckey_num()) {
            del(
                tag_node._slckey_index_ary,
                df_len_of_ary(tag_node._slckey_index_ary),
                this->_slckey_index_ary,
            ...
            del(
                ...
                this->get_slckey_num()+1,
                update_pos);
            tag_node._slckey_index_ary[update_pos-1]  = new_left_key_index;
            tag_node._slcnode_index_ary[update_pos-1] = new_left_slcnode_index;
            tag_node._slcnode_index_ary[update_pos]   = new_right_slcnode_index;
            err = tag_node.set_slckey_num(this->get_slckey_num()-1);
        } else {
            err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_NODE_POS);
        }
...
//
//根节点信息
class dfs_bt_root_t : plclic dfs_bt_const_t {
private:
    //dfs_btree_t * _pbtree;
    //这个多个单根实例...:).
    //fp最高可能到4.
...
class dfs_btree_drill_t : plclic dfs_bt_const_t {
protected:
    int _drill_ptr;
    ...
    uint64_t _slcnode_index_ary[MAX_BT_DEPTH]; //the node history for drilling down...node to drill down..
    uint32_t _ins_pos_ary[MAX_BT_DEPTH]; //insert pos in the node (_slcnode_index_ary[]) of btree
plclic:
    dfs_btree_drill_t() {
        init();
    };
    ...
    bool operator==(const dfs_btree_drill_t & src) const {
        return (_drill_ptr == src._drill_ptr &&
                memcmp(
                    _slcnode_index_ary,
                    src._slcnode_index_ary,
                    (_drill_ptr+1)*sizeof(_slcnode_index_ary[0])) == 0
                && memcmp(
                    _ins_pos_ary,
                    src._ins_pos_ary,
...
plclic:
    ...
    int push(const uint32_t ins_pos, const uint64_t node_index) {
        ...
        if (_drill_ptr < ((int)df_len_of_ary(_slcnode_index_ary)-1)) {
            ++_drill_ptr;
            _slcnode_index_ary[_drill_ptr] = node_index;
    ...
    int pop(void) {
        ...
        if (_drill_ptr >= 0) {
            _slcnode_index_ary[_drill_ptr] = UNDEF_INDEX;
            --_drill_ptr;
            err = 0;
        } else {
...
plclic:
    ...
    uint64_t get_node_index(const int drill_pos) const {
        ...
        if (drill_pos >= 0 && drill_pos <= _drill_ptr) {
            if ((node_index = _slcnode_index_ary[drill_pos]) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
    ...
    uint64_t get_last_node_index(void) const {
        ...
        if (_drill_ptr >= 0 && _drill_ptr < (int)df_len_of_ary(_slcnode_index_ary)) {
            if ((node_index = _slcnode_index_ary[_drill_ptr]) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "node_index == UNDEF_INDEX");
                err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_DRILL_NODE_INDEX);
            }
            ...
            log_level = DF_UL_LOG_TRACE;
            DF_WRITE_LOG_US(log_level, "_drill_ptr < 0 || _drill_ptr >= df_len_of_ary(_slcnode_index_ary)");
            err = DF_BT_SET_NORMAL_ERR(this->ERRNO_BT_DRILL_POS);
        }
...
plclic:
    ...
    int set_del_mid_drill_info(const int del_mid_drill_ptr, const uint64_t del_mid_new_node_index) {
        ...
        if ((_del_mid_drill_ptr = del_mid_drill_ptr) >= 0 && _del_mid_drill_ptr < _drill_ptr) {
            _del_mid_old_node_index = _slcnode_index_ary[_del_mid_drill_ptr];
            _slcnode_index_ary[_del_mid_drill_ptr] = del_mid_new_node_index;
        } else {
...
//对节点更新(插入/删除)后的结果
class dfs_btree_node_mutate_t : plclic dfs_bt_const_t {
plclic:
    enum cconst_plclic {
        //节点操作类型
...
//btree自己保存的头信息，各种不同btree(ID,FP,NS,XZ)还会保存自己的头信息
class dfs_btree_fhead_t : plclic dfs_btree_fhead_base_t, plclic dfs_btree_fhead_ext_t {
plclic:
    enum cconst_plclic {
        STORE_T_INDEX               = 0 ,
        STORE_T_VALUE                   ,
...
class dfs_btree_store_info_t :
    virtual plclic dfs_bt_const_t,
    plclic dfs_s_ary_t<uint64_t> {
plclic:
    enum cconst_plclic {
...
plclic:
    ...
    uint64_t slc_total_key_num(const uint32_t key_num) {
        _total_key_num -= key_num;
        return _total_key_num;
    };
plclic:
...
private:
    //store all contents of a slctree to file
    int _store_slctree(
        const uint64_t slctree_root,
    ...
    int _load_fhead(
        ...
        dfs_btree_fhead_t & btree_fhead) const;
    int _load_slctree(
        uint64_t & slctree_root,
        dfs_btree_load_info_t & load_info);
    int _load_slckeys(
        uint64_t * slckey_index_ary,
...
private:
    ...
    //功能：获得mid node在pos处的slcnode
    //Return: 0 for success, other value for error
    inline int _get_slcnode_of_mid_node(const uint64_t node_index, const uint32_t pos, uint64_t & slcnode_index) const;
private:
...
private:
    ...
    //输入输入：mutate_result
    //返回：0 for success, other values for failure
    int _op_mid_node_of_slc_update(
        const dfs_btree_drill_t & drill_info,
        const int drill_ptr,
        dfs_btree_node_mutate_t & mutate_result);
    ...
    //输入输入：mutate_result
    //返回：0 for success, other values for failure
    int _op_mid_node_of_slc_rebalance_left(
        const dfs_btree_drill_t & drill_info,
        const int drill_ptr,
        dfs_btree_node_mutate_t & mutate_result);
    ...
    //输入输入：mutate_result
    //返回：0 for success, other values for failure
    int _op_mid_node_of_slc_rebalance_right(
        const dfs_btree_drill_t & drill_info,
        const int drill_ptr,
        dfs_btree_node_mutate_t & mutate_result);
    ...
    //输入输入：mutate_result
    //返回：0 for success, other values for failure
    int _op_mid_node_of_slc_merge_left(
        const dfs_btree_drill_t & drill_info,
        const int drill_ptr,
        dfs_btree_node_mutate_t & mutate_result);
    ...
    //输入输入：mutate_result
    //返回：0 for success, other values for failure
    int _op_mid_node_of_slc_merge_right(
        const dfs_btree_drill_t & drill_info,
        const int drill_ptr,
        dfs_btree_node_mutate_t & mutate_result);
    ...
    //输入输入：mutate_result
    //返回：0 for success, other values for failure
    int _op_mid_node_of_slc_merge_both(
        const dfs_btree_drill_t & drill_info,
        const int drill_ptr,
        dfs_btree_node_mutate_t & mutate_result);
private:
    //功能：更新一个叶节点的一个slckey，结果写入mutate_result
    //返回：0 for success, other value for failure
    int _update_leaf_node_slckey(
        const uint64_t node_index,
        const uint32_t ins_pos,
        const uint64_t new_key_index,
        dfs_btree_node_mutate_t & mutate_result);
    //功能：更新一个中间节点的一个slckey，结果写入mutate_result
    //返回：0 for success, other value for failure
    int _update_mid_node_slckey(
        const uint64_t node_index,
        const uint32_t ins_pos,
        const uint64_t new_key_index,
        dfs_btree_node_mutate_t & mutate_result);
...
protected:
    ...
    //  返回：0 for success, other values for error
    int _search_slctree_by_index(
        const dfs_bt_root_t & bt_root,
        const uint64_t slctree_root,
        const uint64_t src_index,    //this object may or may not in the btree
        dfs_btree_drill_t & drill_info,
        uint64_t & obj_index) const;
    int _search_slctree_by_obj(
        const dfs_bt_root_t & bt_root,
        const uint64_t slctree_root,
        const void * pobj,
        dfs_btree_drill_t & drill_info,
        uint64_t & obj_index) const;
    int _search_slctree_by_key(
        const dfs_bt_root_t & bt_root,
        const uint64_t slctree_root,
        const void * pkey,
        dfs_btree_drill_t & drill_info,
...
inline int dfs_btree_t::_get_key_of_node(const uint64_t node_index, const uint32_t pos, uint64_t & key_index) const {
    ...
    } else if (pos >= pnode->get_slckey_num()) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "pos >= pnode->get_slckey_num()");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_POS);
    } else {
        key_index = pnode->get_slckey_index(pos);
    }
...
//功能：获得mid node在pos处的slcnode
//Return: 0 for success, other value for error
inline int dfs_btree_t::_get_slcnode_of_mid_node(const uint64_t node_index, const uint32_t pos, uint64_t & slcnode_index) const {
    const mid_node_t * pmid_node = NULL;
    int log_level = DF_UL_LOG_NONE;
    int err = 0;

    slcnode_index = UNDEF_INDEX;
    if ((err = _get_mid_node(node_index, &pmid_node)) != 0 || NULL == pmid_node) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "_get_mid_node() returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_MID_NODE_NULL_POINTER);
    } else if (pos > pmid_node->get_slckey_num()) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "pos > pmid_node->get_slckey_num()");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_POS);
    } else {
        slcnode_index = pmid_node->get_slcnode_index(pos);
    }
...
class dfs_btree_lock_hold_t : plclic dfs_mutex_hold_t {
};

class dfs_btree_lock_t : plclic dfs_mutex_lock_t {
plclic:
    int init(void) const {
        return 0;
    };
...
int dfs_btree_t::_inc_pointed_ref_counter(const uint64_t final_node_index) {
    ...
    if ((err = _get_node(final_node_index, &pnode)) != 0 || NULL == pnode) {
        ...
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
                          ...
                          pnode->get_slckey_ary(),
                          pnode->get_slckey_num())) != 0) {
        log_level = DF_UL_LOG_FATAL;
        ...
        if ((err = _get_mid_node(final_node_index, &pmid_node)) != 0 || NULL == pmid_node) {
            ...
            //增加每个节点的ref cnt..
            slckey_num = pmid_node->get_slckey_num();
            for (j = 0; j <= slckey_num; ++j) {
                if ((err = _inc_node_ref_counter(pmid_node->get_slcnode_index(j), ref_counter)) != 0) {
                    log_level = DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level, "_inc_node_ref_counter returns 0x%x,", err);
                    break;
                }
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        "node_index=0x%lx, slckey_num=%d",
                        final_node_index, slckey_num);
    }
...
int dfs_btree_t::_dec_pointed_ref_counter(const uint64_t final_node_index) {
    ...
    if ((err = _get_node(final_node_index, &pnode)) != 0 || NULL == pnode) {
        ...
        if ((err = _get_mid_node(final_node_index, &pmid_node)) != 0 || NULL == pmid_node) {
            ...
            for (j = 0; j <= slckey_num; ++j) {
                if ((err = _dec_node_ref_counter(pmid_node->get_slcnode_index(j), ref_counter)) != 0) {
                    log_level = DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level, "_dec_node_ref_counter returns 0x%x", err);
                    DF_WRITE_LOG_US(log_level,
                                    "j=%d, pmid_node->get_slcnode_index(j)=0x%lx",
                                    j, pmid_node->get_slcnode_index(j));
                    break;
                }
            }
        }
...
int dfs_btree_t::_get_drill_last_obj(
    ...
    if (UNDEF_INDEX == last_node_index) {
        ...
        DF_WRITE_LOG_US(log_level, "_get_node() returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
    } else if (last_ins_pos >= (slckey_num = pnode->get_slckey_num())) {
        last_obj_index = UNDEF_INDEX;
    } else if ((last_obj_index = pnode->get_slckey_index(last_ins_pos)) == UNDEF_INDEX) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "pnode->get_slckey_index() returns UNDEF_INDEX");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_KEY_INDEX);
    }

    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        "drill_ptr=%d, last_ins_pos=%d, slckey_num=%d, last_node_index=0x%lx",
                        drill_info.get_drill_ptr(), last_ins_pos, slckey_num, last_node_index);
    }
...
int dfs_btree_t::_update_of_btree(
    ...
    if (UNDEF_POS == myself_ins_pos) {
        ...
        if (_is_mid_node(myself_node_index)) { //中间节点
            if ((err = _update_mid_node_slckey(
                           myself_node_index,
                           myself_ins_pos,
                           new_obj_index,
                           mutate_result)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_update_mid_node_slckey() returns: 0x%x", err);
            }
        } else { //叶节点
            if ((err = _update_leaf_node_slckey(
                           myself_node_index,
                           myself_ins_pos,
                           new_obj_index,
                           mutate_result)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_update_leaf_node_slckey() returns: 0x%x", err);
            }
        }
        if (0 == err) {
            if ((err = _op_tree(
...
int dfs_btree_t::_del_from_btree(
    ...
    uint64_t slctree_largest_obj_index = UNDEF_INDEX;
    ...
    if (UNDEF_POS == myself_ins_pos) {
        ...
        if (_is_mid_node(myself_node_index)) { //中间节点
            ...
            if ((err = _get_mid_node(myself_node_index, &pmid_node)) != 0 || NULL == pmid_node) {
                ...
                DF_WRITE_LOG_US(log_level, "_get_mid_node() returns: 0x%x", err);
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_MID_NODE_NULL_POINTER);
            } else if ((del_node_index = pmid_node->get_slcnode_index(myself_ins_pos)) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "del_node_index == UNDEF_INDEX");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_INDEX);
            } else if ((err = _get_slctree_largest(
                                  del_node_index,
                                  ext_drill_info,
                                  slctree_largest_obj_index)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_get_mid_node() returns: 0x%x", err);
            }
            //Using a reserved node to temporarily keep this node
                ...
                if ((err = pmid_node->update_slckey(
                               *pnew_mid_node,
                               myself_ins_pos,
                               slctree_largest_obj_index)) != 0) {
                    log_level = DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level, "pmid_node->update_slckey() returns: 0x%x", err);
                }
                //重新设置删除的位置....
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        ...
                        mutate_result.get_update_type());
        DF_WRITE_LOG_US(log_level,
                        "new_mid_node_index=0x%lx, slctree_largest_obj_index=0x%lx",
                        new_mid_node_index, slctree_largest_obj_index);
    }
...
int dfs_btree_t::_get_smallest(
    ...
    //得到子树的最小值...
    else if ((err = _get_slctree_smallest(
                        sbt_root.get_root_node_index(),
                        drill_info,
                        obj_index)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "_get_slctree_smallest() returns 0x%x", err);
    } else {
...
int dfs_btree_t::_get_largest(
    ...
    if (UNDEF_INDEX == sbt_root.get_root_node_index()) {
        ...
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_REF_COUNTER);
                          ...
                          obj_index)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "_get_slctree_largest() returns 0x%x", err);
    }
...
//  输出：若存在(子树非空)，则pct指向它且key_index为其值，否则pct为NULL且key_index为UNDEF_INDEX
//  返回: 成功(即使子树空且为根节点)返回0, 其他值错误(ERRNO_BT_DRILL_OBSOLETE则树有新的修改)
int dfs_btree_t::_get_slctree_smallest(
    const uint64_t slctree_root,
    dfs_btree_drill_t & drill_info,
    uint64_t & obj_index) const {
    uint64_t cur_slctree_root = slctree_root;
    uint64_t nex_slctree_root = slctree_root;
    const node_base_t * pnode = NULL;
    const mid_node_t * pmid_node = NULL;
    uint32_t slckey_num = 0;
    int log_level = DF_UL_LOG_NONE;
    ...
    obj_index = UNDEF_INDEX;
    while (0 == err) {
        if ((err = _get_node(cur_slctree_root, &pnode)) != 0 || NULL == pnode) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_get_node() returns 0x%x", err);
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
            break;
        } else if ((slckey_num = pnode->get_slckey_num()) <= 0) {
            //节点没有元素(只有根节点是唯一叶节点时才有这种情况)
            if (drill_info.get_drill_ptr() >= 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "empty btree, but _drill_ptr >= 0");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_POS);
            } else if ((err = drill_info.push(0, cur_slctree_root)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "drill_info.push() returns 0x%x", err);
            }
            break;
        } else if (pnode->is_mid_node()) {
            if ((err = drill_info.push(0, cur_slctree_root)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "drill_info.push() returns 0x%x", err);
            } else if ((err = _get_mid_node(cur_slctree_root, &pmid_node)) != 0 || NULL == pmid_node) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_get_mid_node() returns 0x%x", err);
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_MID_NODE_NULL_POINTER);
                break;
            } else if ((nex_slctree_root = pmid_node->get_slcnode_index(0)) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pmid_node->get_slcnode_index(0) returns UNDEF_INDEX");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_INDEX);
                break;
            } else {
                cur_slctree_root = nex_slctree_root;
            }
        }
        //leaf node, no further search needed
            ...
            if ((err = drill_info.push(0, cur_slctree_root)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "drill_info.push() returns 0x%x", err);
            } else if ((obj_index = pnode->get_slckey_index(0)) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pnode->get_slckey_index(0) returns UNDEF_INDEX");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_INDEX);
            }
            break;
        }
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        "slctree_root=0x%lx, cur_slctree_root=0x%lx, slckey_num=%d",
                        slctree_root, cur_slctree_root, slckey_num);
        DF_WRITE_LOG_US(log_level,
...
//  输出：若存在(子树非空)，则pct指向它且key_index为其值，否则pct为NULL且key_index为UNDEF_INDEX
//  返回: 成功(即使子树空且为根节点)返回0, 其他值错误(ERRNO_BT_DRILL_OBSOLETE则树有新的修改)
int dfs_btree_t::_get_slctree_largest(
    const uint64_t slctree_root,
    dfs_btree_drill_t & drill_info,
    uint64_t & obj_index) const {
    uint64_t cur_slctree_root = slctree_root;
    ...
    while (0 == err) {
        if ((err = _get_node(cur_slctree_root, &pnode)) != 0 || NULL == pnode) {
            ...
            if ((err = drill_info.push(slckey_num, cur_slctree_root)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "drill_info.push() returns 0x%x", err);
                break;
            } else if ((err = _get_mid_node(cur_slctree_root, &pmid_node)) != 0 || NULL == pmid_node) {
                ...
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_MID_NODE_NULL_POINTER);
                break;
            } else if ((nex_slctree_root = pmid_node->get_slcnode_index(slckey_num)) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pmid_node->get_slcnode_index(slckey_num) returns UNDEF_INDEX");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_INDEX);
                break;
            } else {
                cur_slctree_root = nex_slctree_root;
        ...
        //leaf node, no further search needed
            ...
            if ((err = drill_info.push(slckey_num-1, cur_slctree_root)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "drill_info.push() returns 0x%x", err);
                break;
            } else if ((obj_index = pnode->get_slckey_index(slckey_num-1)) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pnode->get_slckey_index(slckey_num-1) returns UNDEF_INDEX");
...
int dfs_btree_t::_get_smaller(
        ...
        if ((last_node_index = drill_info.get_last_node_index()) == UNDEF_INDEX) {
            ...
            if (drill_info.get_drill_ptr() >= 0) {
                if ((next_node_index = drill_info.get_last_node_index()) == UNDEF_INDEX) {
                    ...
                    slckey_num = pnode->get_slckey_num();
                    if ((obj_index = pnode->get_slckey_index(drill_info.get_last_ins_pos()))
                            == UNDEF_INDEX) {
                        log_level = DF_UL_LOG_FATAL;
                        DF_WRITE_LOG_US(log_level,
                                        "pnode->get_slckey_index(drill_info.get_last_ins_pos()) UNDEF_INDEX");
                        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_KEY_INDEX);
                    }
                }
            }
            ...
            if ((err = _get_mid_node(last_node_index, &pmid_node)) != 0 || NULL == pmid_node) {
                ...
                slckey_num = pmid_node->get_slckey_num();
                if ((next_node_index = pmid_node->get_slcnode_index(drill_info.get_last_ins_pos()))
                        == UNDEF_INDEX) {
                    log_level = DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level,
                                    "pmid_node->get_slcnode_index(drill_info.get_last_ins_pos()) returns UNDEF_INDEX");
                    err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCNODE_INDEX);
                } else if ((err = _get_slctree_largest(
                                      next_node_index,
                                      drill_info,
                                      obj_index)) != 0) {
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        "last_node_index=0x%lx, next_node_index=0x%lx, slckey_num=%d",
                        last_node_index, next_node_index, slckey_num);
    }
...
int dfs_btree_t::_get_larger(
    ...
    if (drill_info.get_drill_ptr() >= 0) {
        if ((last_node_index = drill_info.get_last_node_index()) == UNDEF_INDEX) {
            ...
            //叶节点
            if ((drill_info.get_last_ins_pos()+1) < pnode->get_slckey_num()) {
                ...
                while (0 == err) {
                    ...
                    if ((last_node_index = drill_info.get_last_node_index()) == UNDEF_INDEX) {
                        ...
                        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
                        break;
                    } else if (drill_info.get_last_ins_pos() < (slckey_num = pnode->get_slckey_num())) {
                        break;
                    }
                    //到最左边则向根回退上一层
                    else if ((err = drill_info.pop()) != 0) {
            ...
            if (drill_info.get_drill_ptr() >= 0) {
                if ((next_node_index = drill_info.get_last_node_index()) == UNDEF_INDEX) {
                    ...
                    DF_WRITE_LOG_US(log_level, "_get_node() returns 0x%x", err);
                    err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
                } else if ((obj_index = pnode->get_slckey_index(drill_info.get_last_ins_pos()))
                           == UNDEF_INDEX) {
                    log_level = DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level,
                                    "pnode->get_slckey_index(drill_info.get_last_ins_pos()) returns UNDEF_INDEX");
                    err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_INDEX);
                }
            }
        } else {
            //中间节点
            if ((err = _get_mid_node(last_node_index, &pmid_node)) != 0 || NULL == pmid_node) {
                ...
                DF_WRITE_LOG_US(log_level, "_get_mid_node() returns 0x%x", err);
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_MID_NODE_NULL_POINTER);
            } else if (drill_info.get_last_ins_pos() >= pmid_node->get_slckey_num()) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "drill_info.get_last_ins_pos() >= pmid_node->get_slckey_num()");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_POS);
            } else if ((err = drill_info.inc_last_ins_pos()) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "drill_info.inc_last_ins_pos() returns 0x%x", err);
            } else if ((next_node_index = pmid_node->get_slcnode_index(drill_info.get_last_ins_pos()))
                       == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level,
                                "pmid_node->get_slcnode_index(drill_info.get_last_ins_pos()) returns UNDEF_INDEX");
...
//功能：更新一个叶节点的一个slckey，结果写入mutate_result
//返回：0 for success, other value for failure
int dfs_btree_t::_update_leaf_node_slckey(
    const uint64_t node_index,
    const uint32_t ins_pos,
    const uint64_t new_obj_index,
    dfs_btree_node_mutate_t & mutate_result) {
    ...
    if ((err = _get_leaf_node_for_mutate(node_index, &pleaf_node)) != 0 || NULL == pleaf_node) {
        ...
        if ((err = pleaf_node->update_slckey(ins_pos, new_obj_index, old_obj_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pleaf_node->update_slckey() returns 0x%x", err);
        ...
        if ((err = _acquire_leaf_node(new_leaf_node_index, &pnew_leaf_node)) != 0) {
            ...
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
        } else if ((err = pleaf_node->update_slckey(
                              *pnew_leaf_node,
                              ins_pos,
                              new_obj_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
...
//功能：更新一个中间节点的一个slckey，结果写入mutate_result
//返回：0 for success, other value for failure
int dfs_btree_t::_update_mid_node_slckey(
    const uint64_t node_index,
    const uint32_t ins_pos,
    const uint64_t new_obj_index,
    dfs_btree_node_mutate_t & mutate_result) {
...

    {
        ...
        if ((err = pmid_node->update_slckey(ins_pos, new_obj_index, old_obj_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pmid_node->update_slckey() returns 0x%x", err);
        } else if ((err = vir_inc_obj_ref_counter(new_obj_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
        ...
        if ((err = _acquire_mid_node(new_mid_node_index, &pnew_mid_node)) != 0) {
            ...
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_MID_NODE_ACQUIRE);
        } else if ((err = pmid_node->update_slckey(
                              *pnew_mid_node,
                              ins_pos,
                              new_obj_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
...
int dfs_btree_t::_op_tree(
        ...
        //如果为NOTHINg的话,那么就不进行任何的更新.....
    {
        ...
        case    dfs_btree_node_mutate_t::UPDATE_NODE    :
            if ((err = _op_mid_node_of_slc_update(
                           drill_info,
                           cur_drill_ptr,
                           mutate_result)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_op_mid_node_of_slc_update() returns 0x%x", err);
            }
            break;
            //子节点分裂了(产生了两个新节点)
        case    dfs_btree_node_mutate_t::SPLIT_NODE     :
            if ((err = _op_mid_node_of_slc_split(
                           drill_info,
                           cur_drill_ptr,
                           mutate_result)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_op_mid_node_of_slc_split() returns 0x%x", err);
            }
            break;
            //子节点与左兄弟进行了平衡(产生了新的左节点和新的本节点)
        case    dfs_btree_node_mutate_t::REBALANCE_LEFT :
            if ((err = _op_mid_node_of_slc_rebalance_left(
                           drill_info,
                           cur_drill_ptr,
                           mutate_result)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level,
                                "_op_mid_node_of_slc_rebalance_left() returns 0x%x",
                                err);
            }
            break;
            //子节点与右兄弟进行了平衡(产生了新的本节点和新的右节点)
        case    dfs_btree_node_mutate_t::REBALANCE_RIGHT:
            if ((err = _op_mid_node_of_slc_rebalance_right(
                           drill_info,
                           cur_drill_ptr,
                           mutate_result)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level,
                                "_op_mid_node_of_slc_rebalance_right() returns 0x%x",
                                err);
            }
            break;
            //子节点与左兄弟进行了合并
        case    dfs_btree_node_mutate_t::MERGE_LEFT :
            if ((err = _op_mid_node_of_slc_merge_left(
                           drill_info,
                           cur_drill_ptr,
                           mutate_result)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_op_mid_node_of_slc_merge_left() returns 0x%x", err);
            }
            break;
            //子节点与右兄弟进行了合并
        case    dfs_btree_node_mutate_t::MERGE_RIGHT:
            if ((err = _op_mid_node_of_slc_merge_right(
                           drill_info,
                           cur_drill_ptr,
                           mutate_result)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_op_mid_node_of_slc_merge_right() returns 0x%x", err);
            }
            break;
        case    dfs_btree_node_mutate_t::MERGE_BOTH: //子节点与左右兄弟进行了合并
            if ((err = _op_mid_node_of_slc_merge_both(
                           drill_info,
                           cur_drill_ptr,
                           mutate_result)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_op_mid_node_of_slc_merge_both() returns 0x%x", err);
            }
            break;
        default:
            log_level = DF_UL_LOG_FATAL;
...
int dfs_btree_t::_ins_del_leaf_node(
    ...
    if (drill_info.get_drill_ptr() < 0) {
        ...
        if (drill_info.get_del_mid_old_node_index() != UNDEF_INDEX) { //确实要删除...
            ...
            if ((err = pmyself_node->insert_slckey(super_node, myself_ins_pos, ins_obj_index)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pmyself_node->insert_slckey() returns 0x%x", err);
            }
            //得到左右节点信息...
            ...
            if ((err = _acquire_leaf_node(new_myself_node_index, &pnew_myself_node)) != 0) {
                ...
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
            } else if ((err = pmyself_node->insert_slckey(
                                  *pnew_myself_node,
                                  myself_ins_pos,
                                  ins_obj_index)) != 0) {
                log_level = DF_UL_LOG_FATAL;
        ...
        if (pmyself_node->get_slckey_num() <= BT_HALF_FANOUT && drill_info.get_drill_ptr() > 0) {
            //节点半满且非根节点，不能删除项，需与左右兄弟平衡或合并
            if ((err = pmyself_node->del_slckey(super_node, myself_ins_pos)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pmyself_node->del_slckey() returns 0x%x", err);
            } else if ((err = _get_left_right_brothers_info(
                                  left_brother_key_num,
                                  right_brother_key_num,
                                  drill_info,
            ...
            //相反...那么来进行右节点的合并..
                ...
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_UNDERFLOW);
            }
            ...
            if ((err = _acquire_leaf_node(new_myself_node_index, &pnew_myself_node)) != 0) {
                ...
                DF_WRITE_LOG_US(log_level,
                                "UNDEF_INDEX == new_myself_node_index || NULL == pnew_myself_node");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
            } else if ((err = pmyself_node->del_slckey(*pnew_myself_node, myself_ins_pos)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pmyself_node->del_slckey() returns 0x%x", err);
            } else if ((err = _inc_pointed_ref_counter(new_myself_node_index)) != 0) { //由该新节点导致的引用计数增加
                log_level = DF_UL_LOG_FATAL;
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        ...
                        left_brother_key_num, right_brother_key_num);
        DF_WRITE_LOG_US(log_level,
                        "super_node.get_slckey_num()=0x%x",
                        super_node.get_slckey_num());
    }
...
int dfs_btree_t::_get_left_right_brothers_info(
    ...
    if (drill_ptr < 0) {
        ...
        if (myself_in_father_pos > 0) {
            ...
            if ((left_node_index = pfather_node->get_slcnode_index(myself_in_father_pos-1)) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pfather_node->get_slcnode_index(...-1) returns 0x%x", err);
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_INDEX);
                ...
                left_brother_key_num = pleft_node->get_slckey_num();
            }
        }
        if ((myself_in_father_pos+1) <= pfather_node->get_slckey_num()) {
            //right brother: if there is a right brother
            if ((right_node_index = pfather_node->get_slcnode_index(myself_in_father_pos+1)) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pfather_node->get_slcnode_index(...+1) returns 0x%x", err);
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_INDEX);
                ...
                right_brother_key_num = pright_node->get_slckey_num();
            }
        }
    }
...
int dfs_btree_t::_leaf_rebalance_left(
    ...
    if (drill_ptr <= 0) {
        ...
        DF_WRITE_LOG_US(log_level, "_get_mid_node() returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_NODE_NULL_POINTER);
    } else if (myself_in_father_pos > pfather_node->get_slckey_num() || //插入位置不能超出父节点的slckey_num
               myself_in_father_pos <= 0) { //there should be 1+ left brother
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "myself_in_father_pos > pfather_node->get_slckey_num() || <= 0");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_INS_POS);
    } else if ((left_brother_key_index = pfather_node->get_slckey_index(myself_in_father_pos-1))
               == UNDEF_INDEX) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "left_brother_key_index == UNDEF_INDEX");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_INDEX);
    } else if ((left_brother_node_index = pfather_node->get_slcnode_index(myself_in_father_pos-1))
               == UNDEF_INDEX) {
        log_level = DF_UL_LOG_FATAL;
        ...
        DF_WRITE_LOG_US(log_level, "_get_leaf_node() returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_NULL_POINTER);
    } else if ((err = pleft_brother_node->insert_slckey(
                          key_index_ary+slckey_num,
                          df_len_of_ary(key_index_ary)-slckey_num,
                          pleft_brother_node->get_slckey_num(),
                          left_brother_key_index)) != 0)
        //把left brother对应的key_index插入到最后
    {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "pleft_brother_node->insert_slckey() returns 0x%x", err);
    } else {
        slckey_num += pleft_brother_node->get_slckey_num() + 1;
        //也是copy过去...
        if ((err = updated_myself_node.get_slckeys(
                       key_index_ary+slckey_num,
                       df_len_of_ary(key_index_ary)-slckey_num)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "updated_myself_node.get_slckeys() returns 0x%x", err);
        } else {
            slckey_num += updated_myself_node.get_slckey_num();

            if (slckey_num < (BT_HALF_FANOUT*2+1)) {
                //下溢
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "slckey_num < (BT_HALF_FANOUT*2+1)");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_UNDERFLOW);
            } else if (slckey_num > (2*(BT_FANOUT-1)+1)) {
                //上溢
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "slckey_num > (2*(BT_FANOUT-1)+1)");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_OVERFLOW);
            }
        }
    }
    ...
    if (0 == err) { //不论是否checkpointing都得使用新节点而不能直接修改原有节点
        ...
        if ((err = _acquire_leaf_node(new_left_node_index, &pnew_left_node)) != 0) {
            ...
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
        } else if ((err = pnew_left_node->put_slckeys(key_index_ary, ptr)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pnew_left_node->put_slckeys() returns 0x%x", err);
        } else if ((err = pnew_myself_node->put_slckeys(key_index_ary+ptr+1, slckey_num-(ptr+1))) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pnew_myself_node->put_slckeys() returns 0x%x", err);
        }
        //从最上面加一次引用计数...
        //写内容...
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        "drill_ptr=%d, myself_in_father_pos=%d, slckey_num=%d",
                        drill_ptr, myself_in_father_pos, slckey_num);
        DF_WRITE_LOG_US(log_level,
                        "father_node_index=0x%lx, pfather_node=0x%p",
                        father_node_index, pfather_node);
        DF_WRITE_LOG_US(log_level,
        ...
        BT_DF_WRITE_LOG_INT64S(log_level, "key_index_ary", key_index_ary, slckey_num);
    }
...
int dfs_btree_t::_mid_rebalance_left(
        ...
        //把left brother对应的key_index插入到最后
    {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "pleft_brother_node->insert_slckey() returns 0x%x", err);
    } else if ((err = pleft_brother_node->export_slcnode_index(
                          node_index_ary+slckey_num,
                          df_len_of_ary(node_index_ary)-slckey_num)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "pleft_brother_node->export_slcnode_index() returns 0x%x", err);
    } else {
        slckey_num += pleft_brother_node->get_slckey_num() + 1;
        ...
        if ((err = updated_myself_node.get_slckeys(
            ...
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "updated_myself_node.get_slckeys() returns 0x%x", err);
        } else if ((err = updated_myself_node.export_slcnode_index(
                              node_index_ary+slckey_num,
                              df_len_of_ary(node_index_ary)-slckey_num)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "updated_myself_node.export_slcnode_index() returns 0x%x", err);
        } else {
            slckey_num += updated_myself_node.get_slckey_num();
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        ...
                        left_brother_key_index);
        BT_DF_WRITE_LOG_INT64S(log_level, "key_index_ary", key_index_ary, slckey_num);
        BT_DF_WRITE_LOG_INT64S(log_level, "node_index_ary", node_index_ary, slckey_num+1);
    }
...
int dfs_btree_t::_leaf_rebalance_right(
    ...
    if (drill_ptr <= 0) {
        ...
        DF_WRITE_LOG_US(log_level, "_get_mid_node() returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_MID_NODE_NULL_POINTER);
    } else if ((myself_in_father_pos+1) > pfather_node->get_slckey_num()) { //should be 1+ right brothe
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "(myself_in_father_pos+1) > pfather_node->get_slckey_num()");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_INS_POS);
    } else if ((myself_key_index = pfather_node->get_slckey_index(myself_in_father_pos))
               == UNDEF_INDEX) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "myself_key_index == UNDEF_INDEX");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_INDEX);
    } else if ((right_brother_node_index = pfather_node->get_slcnode_index(myself_in_father_pos+1))
               == UNDEF_INDEX) {
        log_level = DF_UL_LOG_FATAL;
        ...
        DF_WRITE_LOG_US(log_level, "_get_leaf_node() returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_NULL_POINTER);
    } else if ((err = updated_myself_node.insert_slckey(
                          key_index_ary+slckey_num,
                          df_len_of_ary(key_index_ary)-slckey_num,
                          updated_myself_node.get_slckey_num(),
                          myself_key_index)) != 0)
        //把myself对应的key_index插入到最后
    {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "updated_myself_node.insert_slckey() returns 0x%x", err);
    } else {
        slckey_num += updated_myself_node.get_slckey_num() + 1;

        if ((err = pright_brother_node->get_slckeys(
                       key_index_ary+slckey_num,
                       df_len_of_ary(key_index_ary)-slckey_num)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pright_brother_node->get_slckeys() returns 0x%x", err);
        } else {
            slckey_num += pright_brother_node->get_slckey_num();
    ...
    if (0 == err) {  //不论是否checkpointing都得使用新节点而不能直接修改原有节点
        ...
        if ((err = _acquire_leaf_node(new_myself_node_index, &pnew_myself_node)) != 0) {
            ...
            DF_WRITE_LOG_US(log_level,
                            "UNDEF_INDEX == new_myself_node_index || NULL == pnew_myself_node");
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
        } else if ((err = pnew_myself_node->put_slckeys(key_index_ary, ptr)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pnew_myself_node->put_slckeys() returns 0x%x", err);
        } else if ((err = pnew_right_node->put_slckeys(
                              key_index_ary+ptr+1,
                              slckey_num-(ptr+1))) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pnew_right_node->put_slckeys() returns 0x%x", err);
        } else if ((err = _inc_pointed_ref_counter(new_myself_node_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_inc_pointed_ref_counter(new_myself_node_index) returns 0x%x", err);
...
int dfs_btree_t::_mid_rebalance_right(
        ...
        //把myself对应的key_index插入到最后
    {
        ...
        if ((err = pright_brother_node->get_slckeys(
            ...
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pright_brother_node->get_slckeys() returns 0x%x", err);
        } else if ((err = pright_brother_node->export_slcnode_index(
                              node_index_ary+slckey_num,
                              df_len_of_ary(node_index_ary)-slckey_num)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pright_brother_node->export_slcnode_index() returns 0x%x", err);
        } else {
            slckey_num += pright_brother_node->get_slckey_num();
...
int dfs_btree_t::_leaf_split(
    ...
    if (drill_ptr < 0) {
        ...
        DF_WRITE_LOG_US(log_level, "drill_ptr < 0");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_POS);
    } else if (updated_myself_node.get_slckey_num() < (BT_HALF_FANOUT*2+1)) {
        //下溢
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "updated_myself_node.get_slckey_num() < (BT_HALF_FANOUT*2+1)");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_UNDERFLOW);
    } else if ((err = updated_myself_node.get_slckeys(
                          key_index_ary+slckey_num,
                          df_len_of_ary(key_index_ary)-slckey_num)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "updated_myself_node.get_slckeys() returns 0x%x", err);
    ...
    if (0 == err) {  //不论是否checkpointing都得使用新节点而不能直接修改原有节点
        ...
        if ((err = _acquire_leaf_node(new_left_node_index, &pnew_left_node)) != 0) {
            ...
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pnew_left_node->put_slckeys() returns 0x%x", err);
        } else if ((err = pnew_right_node->put_slckeys(key_index_ary+ptr+1, slckey_num-(ptr+1))) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pnew_right_node->put_slckeys() returns 0x%x", err);
        } else if ((err = _inc_pointed_ref_counter(new_left_node_index)) != 0) { //???..
            log_level = DF_UL_LOG_FATAL;
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        "drill_ptr=%d, slckey_num=%d",
                        drill_ptr, slckey_num);
        DF_WRITE_LOG_US(log_level,
                        "updated_myself_node.get_slckey_num()=%d",
                        updated_myself_node.get_slckey_num());
        BT_DF_WRITE_LOG_INT64S(log_level, "key_index_ary", key_index_ary, slckey_num);
    }
...
int dfs_btree_t::_leaf_merge_left(
        ...
        //把left brother对应的key_index插入到最后
    {
        ...
        if ((err = updated_myself_node.get_slckeys(
            ...
            if (slckey_num < BT_HALF_FANOUT) {
                //下溢
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "slckey_num < BT_HALF_FANOUT");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_UNDERFLOW);
            } else if (slckey_num > (BT_FANOUT-1)) {
                //上溢
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "slckey_num > (BT_FANOUT-1)");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_OVERFLOW);
            }
        }
    }
    ...
    if (0 == err) { //不论是否checkpointing都得使用新节点而不能直接修改原有节点
        ...
        if ((err = _acquire_leaf_node(new_myself_node_index, &pnew_myself_node)) != 0) {
            ...
            DF_WRITE_LOG_US(log_level, "UNDEF_INDEX == new_myself_node_index || NULL == pnew_myself_node");
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
        } else if ((err = pnew_myself_node->put_slckeys(key_index_ary, slckey_num)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pnew_myself_node->put_slckeys() returns 0x%x", err);
        } else if ((err = _inc_pointed_ref_counter(new_myself_node_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        ...
                        left_brother_node_index, pleft_brother_node);
        DF_WRITE_LOG_US(log_level,
                        "left_brother_key_index=0x%lx, updated_myself_node..get_slckey_num()=%d",
                        left_brother_key_index, updated_myself_node.get_slckey_num());
        BT_DF_WRITE_LOG_INT64S(log_level, "key_index_ary", key_index_ary, slckey_num);
    }
...
int dfs_btree_t::_leaf_merge_right(
    ...
    if (drill_ptr <= 0) {
        ...
        DF_WRITE_LOG_US(log_level, "(myself_in_father_pos+1) > pfather_node->get_slckey_num()");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_INS_POS);
    } else if ((myself_key_index = pfather_node->get_slckey_index(myself_in_father_pos)) == UNDEF_INDEX) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "myself_key_index == UNDEF_INDEX");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_INDEX);
    } else if ((right_brother_node_index = pfather_node->get_slcnode_index(myself_in_father_pos+1))
    ...
    if (0 == err) {  //不论是否checkpointing都得使用新节点而不能直接修改原有节点
        ...
        if ((err = _acquire_leaf_node(new_right_node_index, &pnew_right_node)) != 0) {
            ...
            DF_WRITE_LOG_US(log_level, "UNDEF_INDEX == new_right_node_index || NULL == pnew_right_node");
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
        } else if ((err = pnew_right_node->put_slckeys(key_index_ary, slckey_num)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pnew_right_node->put_slckeys() returns 0x%x", err);
        } else if ((err = _inc_pointed_ref_counter(new_right_node_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        ...
                        right_brother_node_index, pright_brother_node);
        DF_WRITE_LOG_US(log_level,
                        "myself_key_index=0x%lx, updated_myself_node..get_slckey_num()=%d",
                        myself_key_index, updated_myself_node.get_slckey_num());
        BT_DF_WRITE_LOG_INT64S(log_level, "key_index_ary", key_index_ary, slckey_num);
    }
...
int dfs_btree_t::_leaf_merge_both(
    ...
    if (drill_ptr <= 0) {
        ...
        DF_WRITE_LOG_US(log_level, "_get_mid_node(father_node_index...) returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
    } else if (myself_in_father_pos <= 0 || (myself_in_father_pos+1) > pfather_node->get_slckey_num())
        //there should be 1+ left and 1+ right brother
    {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "myself_in_father_pos <= 0 || (...+1) > pfather_node->get_slckey_num()");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_INS_POS);
    } else if ((left_brother_key_index = pfather_node->get_slckey_index(myself_in_father_pos-1))
               == UNDEF_INDEX) {
        log_level = DF_UL_LOG_FATAL;
        ...
        //把left brother对应的key_index插入到最后
    {
        ...
        if ((err = updated_myself_node.insert_slckey(
                       key_index_ary+slckey_num,
                       df_len_of_ary(key_index_ary)-slckey_num,
                       updated_myself_node.get_slckey_num(),
                       myself_key_index)) != 0)
...
//说明：若是因为删除导致，则此时myself的key_index可能需要替换
//返回：0 for success, other values for failure
int dfs_btree_t::_op_mid_node_of_slc_update(
    const dfs_btree_drill_t & drill_info,
    const int drill_ptr,
    dfs_btree_node_mutate_t & mutate_result) {
    uint64_t new_slcnode_index = UNDEF_INDEX;
    uint64_t old_slcnode_index = UNDEF_INDEX;
    uint64_t new_slcnode_ref_counter = UNDEF_REF_COUNTER;
    uint64_t old_slcnode_ref_counter = UNDEF_REF_COUNTER;
    uint64_t myself_node_index = UNDEF_INDEX;
    ...
    if ((err = mutate_result.get_update_node_info(new_slcnode_index)) != 0) {
        log_level = DF_UL_LOG_FATAL;
    ...
    //可以直接进行修改...
        ...
        //直接修改
        if ((err = pmyself_node->update_slcnode(
                       myself_ins_pos,
                       new_slcnode_index,
                       old_slcnode_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pmyself_node->update_slcnode() returns 0x%x", err);
        }
        //增加...
        else if ((err = _inc_node_ref_counter(new_slcnode_index, new_slcnode_ref_counter)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_inc_node_ref_counter(new_slcnode_index) returns 0x%x", err);
        } else if ((err = _dec_node_ref_counter(old_slcnode_index, old_slcnode_ref_counter)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_dec_node_ref_counter(old_slcnode_index) returns 0x%x", err);
        } else {
            //设置0...
            mutate_result.set_mutate_nothing();
            err = 0;
        ...
        if ((err = _acquire_mid_node(new_myself_node_index, &pnew_myself_node)) != 0) {
            ...
            DF_WRITE_LOG_US(log_level, "UNDEF_INDEX == new_myself_node_index || NULL == pnew_myself_node");
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
        } else if ((err = pmyself_node->update_slcnode(*pnew_myself_node,
                          myself_ins_pos,
                          new_slcnode_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pmyself_node->update_slcnode() returns 0x%x", err);
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        "new_slcnode_index=0x%lx, old_slcnode_index=0x%lx",
                        new_slcnode_index, old_slcnode_index);
        DF_WRITE_LOG_US(log_level,
                        "new_slcnode_ref_counter=%ld, old_slcnode_ref_counter=%ld",
                        new_slcnode_ref_counter, old_slcnode_ref_counter);
        DF_WRITE_LOG_US(log_level,
...
//说明：若是因为删除导致，则此时myself的key_index可能需要替换
//返回：0 for success, other values for failure
int dfs_btree_t::_op_mid_node_of_slc_rebalance_left(
    const dfs_btree_drill_t & drill_info,
    const int drill_ptr,
    dfs_btree_node_mutate_t & mutate_result) {
    uint64_t new_left_slckey_index = UNDEF_INDEX;
    uint64_t new_left_slcnode_index = UNDEF_INDEX;
    uint64_t new_myself_slcnode_index = UNDEF_INDEX;
    uint64_t myself_node_index = UNDEF_INDEX;
    ...
    if ((err = mutate_result.get_rebalance_left_info(
                   new_left_slckey_index,
                   new_left_slcnode_index,
                   new_myself_slcnode_index)) != 0) {
        log_level = DF_UL_LOG_FATAL;
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        ...
                        myself_node_index, pmyself_node);
        DF_WRITE_LOG_US(log_level,
                        "new_left_slckey_index=0x%lx, new_left_slcnode_index=0x%lx",
                        new_left_slckey_index, new_left_slcnode_index);
        DF_WRITE_LOG_US(log_level,
                        "new_myself_slcnode_index=0x%lx",
                        new_myself_slcnode_index);
        DF_WRITE_LOG_US(log_level,
                        "mutate_result.get_update_type()=%d",
                        mutate_result.get_update_type());
    }
...
//输入输入：mutate_result
//返回：0 for success, other values for failure
int dfs_btree_t::_op_mid_node_of_slc_rebalance_right(
    const dfs_btree_drill_t & drill_info,
    const int drill_ptr,
    dfs_btree_node_mutate_t & mutate_result) {
    uint64_t new_myself_slckey_index = UNDEF_INDEX;
    uint64_t new_myself_slcnode_index = UNDEF_INDEX;
    uint64_t new_right_slcnode_index = UNDEF_INDEX;
    uint64_t myself_node_index = UNDEF_INDEX;
    const mid_node_t * pmyself_node = NULL;
    uint32_t myself_ins_pos = UNDEF_POS;
    int log_level = DF_UL_LOG_NONE;
    ...
    if ((err = mutate_result.get_rebalance_right_info(
                   new_myself_slckey_index,
                   new_myself_slcnode_index,
                   new_right_slcnode_index)) != 0) {
        log_level = DF_UL_LOG_FATAL;
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        ...
                        myself_node_index, pmyself_node);
        DF_WRITE_LOG_US(log_level,
                        "new_myself_slckey_index=0x%lx, new_myself_slcnode_index=0x%lx",
                        new_myself_slckey_index, new_myself_slcnode_index);
        DF_WRITE_LOG_US(log_level,
                        "new_right_slcnode_index=0x%lx",
                        new_right_slcnode_index);
        DF_WRITE_LOG_US(log_level,
                        "mutate_result.get_update_type()=%d",
                        mutate_result.get_update_type());
    }
...
//输入输入：mutate_result
//返回：0 for success, other values for failure
int dfs_btree_t::_op_mid_node_of_slc_split(
    const dfs_btree_drill_t & drill_info,
    const int drill_ptr,
    dfs_btree_node_mutate_t & mutate_result) {
    uint64_t new_left_slckey_index = UNDEF_INDEX;
...
//输入输入：mutate_result
//返回：0 for success, other values for failure
int dfs_btree_t::_op_mid_node_of_slc_merge_left(
    const dfs_btree_drill_t & drill_info,
    const int drill_ptr,
    dfs_btree_node_mutate_t & mutate_result) {
    uint64_t new_myself_slcnode_index = UNDEF_INDEX;
    ...
    if ((err = mutate_result.get_merge_left_info(new_myself_slcnode_index)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        ...
        DF_WRITE_LOG_US(log_level, "_get_mid_node(myself_node_index...) returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
    } else if (myself_ins_pos <= 0 || myself_ins_pos > pmyself_node->get_slckey_num()) {
        //at least one left brother
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "myself_ins_pos <= 0 || ... > pmyself_node->get_slckey_num()");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_INS_POS);
    }
    //不能用原子操作一次更新，必须copy-on-write
    else if (pmyself_node->get_slckey_num() <= BT_HALF_FANOUT && drill_ptr > 0) { //节点下溢，需要与左兄弟或右兄弟平衡或合并
        uint32_t left_brother_key_num  = 0;
        uint32_t right_brother_key_num = 0;
        dfs_btree_mid_node_t<BT_FANOUT+2> super_node;
    ...
    } else if (pmyself_node->get_slckey_num() > 1) { //节点未下溢或者是根节点且slckey_num>1，直接删除
        mid_node_t * pnew_myself_node = NULL;
        uint64_t new_myself_node_index = UNDEF_INDEX;
    ...
    } else { //根节点且slckey_num == 1
        mutate_result.set_update_node_info(new_myself_slcnode_index);
        err = 0;
    }
...
//输入输入：mutate_result
//返回：0 for success, other values for failure
int dfs_btree_t::_op_mid_node_of_slc_merge_right(
    const dfs_btree_drill_t & drill_info,
    const int drill_ptr,
    dfs_btree_node_mutate_t & mutate_result) {
    uint64_t new_right_slcnode_index = UNDEF_INDEX;
    ...
    if ((err = mutate_result.get_merge_right_info(new_right_slcnode_index)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        ...
        DF_WRITE_LOG_US(log_level, "_get_mid_node(myself_node_index...) returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
    } else if ((myself_ins_pos+1) > pmyself_node->get_slckey_num()) {
        //at least one right brother
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "(myself_ins_pos+1) > pmyself_node->get_slckey_num()");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_INS_POS);
    }
    //不能用原子操作一次更新，必须copy-on-write
    else if (pmyself_node->get_slckey_num() <= BT_HALF_FANOUT && drill_ptr > 0) { //节点下溢，需要与左兄弟或右兄弟平衡或合并
        ...
        mutate_result.set_update_node_info(new_right_slcnode_index);
        err = 0;
    }
...
//输入输入：mutate_result
//返回：0 for success, other values for failure
int dfs_btree_t::_op_mid_node_of_slc_merge_both(
    const dfs_btree_drill_t & drill_info,
    const int drill_ptr,
    dfs_btree_node_mutate_t & mutate_result) {
    ...
    if ((err = mutate_result.get_merge_both_info(
        ...
        DF_WRITE_LOG_US(log_level, "_get_mid_node(myself_node_index...) returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
    } else if (myself_ins_pos <= 0 || (myself_ins_pos+1) > pmyself_node->get_slckey_num()) {
        //at least one left brother and one right brother
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "myself_ins_pos <= 0 || (...+1) > pmyself_node->get_slckey_num()");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_INS_POS);
    }
    //不能用原子操作一次更新，必须copy-on-write
    else if (pmyself_node->get_slckey_num() <= BT_HALF_FANOUT && drill_ptr > 0) { //节点下溢，需要与左兄弟或右兄弟平衡或合并
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        ...
                        myself_node_index, pmyself_node);
        DF_WRITE_LOG_US(log_level,
                        "new_left_key_index=0x%lx, new_left_slcnode_index=0x%lx",
                        new_left_key_index, new_left_slcnode_index);
        DF_WRITE_LOG_US(log_level,
                        "new_right_slcnode_index=0x%lx",
                        new_right_slcnode_index);
        DF_WRITE_LOG_US(log_level,
...
int dfs_btree_t::_search_by_index(
    ...
    if (sbt_root.get_root_node_index() == UNDEF_INDEX) {
        ...
        DF_WRITE_LOG_US(log_level, "ref_counter <= 0");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_REF_COUNTER);
    } else if ((err = _search_slctree_by_index(
                          bt_root,
                          sbt_root.get_root_node_index(),
                          i_obj_index,
                          drill_info,
                          tag_obj_index)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "_search_slctree() returns 0x%x", err);
    } else if (drill_info.get_drill_ptr() < 0) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "drill_info.get_drill_ptr() < 0");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_DRILL_POS);
...
int dfs_btree_t::_search_by_obj(
    ...
    if (sbt_root.get_root_node_index() == UNDEF_INDEX) {
        ...
        DF_WRITE_LOG_US(log_level, "ref_counter <= 0");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_REF_COUNTER);
    } else if ((err = _search_slctree_by_obj(
                          bt_root,
                          sbt_root.get_root_node_index(),
                          pobj,
                          drill_info,
...
int dfs_btree_t::_search_by_key(
    ...
    if (sbt_root.get_root_node_index() == UNDEF_INDEX) {
        ...
        DF_WRITE_LOG_US(log_level, "ref_counter <= 0");
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_REF_COUNTER);
    } else if ((err = _search_slctree_by_key(
                          bt_root,
                          sbt_root.get_root_node_index(),
                          pkey,
                          drill_info,
...
//      如果该元素存在，则drill_info中信息指向它，否则指向它插入后的位置。
//  return: 0 for success, other values for error
int dfs_btree_t::_search_slctree_by_index(
    const dfs_bt_root_t & bt_root,
    const uint64_t slctree_root,
    ...
    while (UNDEF_INDEX == obj_index) {
        ...
        if ((err = _search_node_by_index(
            ...
            break;
        } else if (UNDEF_INDEX == obj_index && _is_mid_node(cur_slctree_root)) {
            //not found & is_mid_node
            if ((err = _get_mid_node(cur_slctree_root, &pmid_node)) != 0 || NULL == pmid_node) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_get_mid_node() returns 0x%x", err);
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
                break;
            }
            //需要在这个slc index下面继续find...
            else if ((cur_slctree_root =
                          pmid_node->get_slcnode_index(drill_info.get_last_ins_pos())) == UNDEF_INDEX) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "pmid_node->get_slcnode_index() returns 0x%x", err);
                DF_WRITE_LOG_US(log_level,
                                "last_ins_pos=%d; get_slckey_num()=%d",
                                drill_info.get_last_ins_pos(), pmid_node->get_slckey_num());
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_INDEX);
                break;
            }
        } else { //叶子节点或者是found...
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        "src_index=0x%lx, slctree_root=0x%lx, drill_ptr=%d",
                        src_index, slctree_root, drill_info.get_drill_ptr());
    }

    return err;
};
int dfs_btree_t::_search_slctree_by_obj(
    const dfs_bt_root_t & bt_root,
    const uint64_t slctree_root,
    const void * pobj, //may not in the tree
    dfs_btree_drill_t & drill_info,
    ...
    while (UNDEF_INDEX == obj_index) {
        //搜索结构自动追加到drill_info
        if ((err = _search_node_by_obj(bt_root, cur_slctree_root, pobj, drill_info, obj_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_search_node() returns 0x%x", err);
            break;
        } else if (UNDEF_INDEX == obj_index && _is_mid_node(cur_slctree_root)) {
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        "pobj=0x%p, slctree_root=0x%lx",
                        pobj, slctree_root);
        DF_WRITE_LOG_US(log_level,
                        "cur_slctree_root=0x%lx, drill_ptr=%d",
                        cur_slctree_root, drill_info.get_drill_ptr());
    }

    return err;
};
int dfs_btree_t::_search_slctree_by_key(
    const dfs_bt_root_t & bt_root,
    const uint64_t slctree_root,
    const void * pkey, //may not in the tree
    dfs_btree_drill_t & drill_info,
    ...
    while (UNDEF_INDEX == obj_index) {
        //搜索结构自动追加到drill_info
        if ((err = _search_node_by_key(bt_root, cur_slctree_root, pkey, drill_info, obj_index)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_search_node() returns 0x%x", err);
            break;
        } else if (UNDEF_INDEX == obj_index && _is_mid_node(cur_slctree_root)) {
...
int dfs_btree_t::_search_node_by_index(
    ...
    if ((err = _get_node(node_index, &pnode)) != 0 || NULL == pnode) {
        ...
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
                          ...
                          bt_root,
                          pnode->get_slckey_ary(),
                          pnode->get_slckey_num(),
                          src_index,
                          ins_pos,
                          obj_index)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "vir_search_ary_by_index() returns 0x%x", err);
        DF_WRITE_LOG_US(log_level, "slckey_num=%d", pnode->get_slckey_num());
    } else {
...
//      right_brother_key_index：子树的右兄弟，如果不等于UNDEF_INDEX，则>=子树的所有项
//返回：返回0如果验证通过，返回非0(错误代码)如果验证失败
int dfs_btree_t::_verify_slctree(
    const dfs_bt_root_t & bt_root,
    const uint64_t /*father_mutation_counter*/,
    const uint64_t slctree_root,
    ...
    if ((err = _get_node(slctree_root, &pnode)) != 0 || NULL == pnode) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "_get_node() returns 0x%x", err);
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
    } else if (pnode->get_slckey_num() > (BT_FANOUT-1)) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "pnode->get_slckey_num() > (BT_FANOUT-1)");
        DF_WRITE_LOG_US(log_level, "pnode->get_slckey_num()=%d", pnode->get_slckey_num());
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_OVERFLOW);
    } else if (pnode->get_slckey_num() < (BT_FANOUT-1)/2 &&
        ...
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "pnode->get_slckey_num() < (BT_FANOUT-1)/2 && ...");
        DF_WRITE_LOG_US(log_level, "pnode->get_slckey_num()=%d", pnode->get_slckey_num());
        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_UNDERFLOW);
    } else {
        j = 0;
        key_index_ary[j++] = left_brother_key_index;
        if ((err = pnode->get_slckeys(key_index_ary+j, df_len_of_ary(key_index_ary)-j)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pnode->get_slckeys() returns 0x%x", err);
        } else {
            j += pnode->get_slckey_num();
        ...
        if (0 == err) {
            ...
            //同时判断是否为升序...
                ...
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_ORDER);
            }
        }
        if (0 == err && _is_mid_node(slctree_root)) {
            const mid_node_t * pmid_node = NULL;
            uint64_t slcnode_index = UNDEF_INDEX;

            if ((err = _get_mid_node(slctree_root, &pmid_node)) != 0 || NULL == pmid_node) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_get_mid_node(slctree_root...) returns 0x%x", err);
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
            } else {
                n = pnode->get_slckey_num()+1;
                for (j = 0; j < n; ++j) {
                    if ((slcnode_index = pmid_node->get_slcnode_index(j)) == UNDEF_INDEX) {
                        log_level = DF_UL_LOG_FATAL;
                        DF_WRITE_LOG_US(log_level, "slcnode_index == UNDEF_INDEX");
                        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCNODE_INDEX);
                        break;
                    } else if (_is_leaf_node(pmid_node->get_slcnode_index(0)) &&
                               !_is_leaf_node(slcnode_index)) {
                        log_level = DF_UL_LOG_FATAL;
                        DF_WRITE_LOG_US(log_level, "_is_leaf_node(slcnode_index[0]) && !_is_leaf_node(slcnode_index)");
                        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_HETEROGENEOUS_MID_NODE);
                        break;
                    } else if (_is_mid_node(pmid_node->get_slcnode_index(0)) &&
                               !_is_mid_node(slcnode_index)) {
                        log_level = DF_UL_LOG_FATAL;
                        DF_WRITE_LOG_US(log_level, "_is_mid_node(slcnode_index[0]) && !_is_mid_node(slcnode_index)");
                        err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_HETEROGENEOUS_MID_NODE);
                        break;
                    } else if ((err = _verify_slctree(
                                          bt_root,
                                          mutation_counter,
                                          slcnode_index,
                                          key_index_ary[j],
                                          key_index_ary[j+1])) != 0) {
                        log_level = DF_UL_LOG_FATAL;
                        DF_WRITE_LOG_US(log_level, "_verify_slctree() returns 0x%x", err);
                        break;
                    }
                }
            }
            ...
            if (DF_UL_LOG_NONE != log_level) {
                DF_WRITE_LOG_US(log_level,
                                "slcnode_index=0x%lx, pmid_node=0x%p",
                                slcnode_index, pmid_node);
            }
        }
    }
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        "slctree_root=0x%lx, left_brother_key_index=0x%lx, right_brother_key_index=0x%lx",
                        slctree_root, left_brother_key_index, right_brother_key_index);
        DF_WRITE_LOG_US(log_level,
...
//Return: 0 for success, other values for error
int dfs_btree_t::_get_slctree_total_num(
    const uint64_t slctree_root,
    uint64_t & total_key_num,
    uint64_t & total_leaf_node_num,
    uint64_t & total_mid_node_num) const {
    ...
    if (_is_mid_node(slctree_root)) {
        if ((err = _get_mid_node(slctree_root, &pmid_node)) != 0 || NULL == pmid_node) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_get_mid_node(slctree_root...) returns 0x%x", err);
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
        } else {
            total_key_num += pmid_node->get_slckey_num();
            n = pmid_node->get_slckey_num()+1;
            for (j = 0; j < n; ++j) {
                if ((slcnode_index = pmid_node->get_slcnode_index(j)) == UNDEF_INDEX) {
                    log_level = DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level, "slcnode_index == UNDEF_INDEX");
                    err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCNODE_INDEX);
                    break;
                } else if ((err = _get_slctree_total_num(
                                      slcnode_index,
                                      total_key_num,
                                      total_leaf_node_num,
                                      total_mid_node_num)) != 0) {
                    log_level = DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level, "_get_slctree_total_num() returns 0x%x", err);
                    break;
                }
            }
        }
        ...
        if ((err = _get_leaf_node(slctree_root, &pleaf_node)) != 0 || NULL == pleaf_node) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_get_leaf_node(slctree_root...) returns 0x%x", err);
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
        } else {
            total_key_num += pleaf_node->get_slckey_num();
        }
        ++total_leaf_node_num;
    }
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        "slctree_root=0x%lx, slcnode_index=0x%lx",
                        slctree_root, slcnode_index);
        DF_WRITE_LOG_US(log_level,
...
int dfs_btree_t::_bt_store_tree(
    ...
    //空的树也要保存文件后和节点信息
    else { //if (pnode->get_slckey_num() >= 0)
        //首先写头...
        //保存一下是在什么位置上面改写的:)..
        if ((err = _store_fhead(store_info, file_head_offset, btree_fhead)) != 0) {
            ...
            DF_WRITE_LOG_US(log_level,
                            ...
                            sbt_root.get_root_node_index(), sbt_root.get_mutation_counter());
            //然后保存子树...
            if ((err = _store_slctree(sbt_root.get_root_node_index(), store_info)) != 0) {
                log_level = (ERRNO_BT_CKP_CANCELLED == err) ? DF_UL_LOG_NOTICE : DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_store_slctree() returns 0x%x", err);
            }
...
int dfs_btree_t::_bt_load_tree(
    ...
    //会得到load info的index...:)..
    else if ((err = _load_slctree(new_root_index, load_info)) != 0) {
        log_level = (ERRNO_BT_CKP_CANCELLED == err) ? DF_UL_LOG_NOTICE : DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "_load_slctree() returns 0x%x", err);
...
//store all contents of a slctree to file
int dfs_btree_t::_store_slctree(
    const uint64_t slctree_root,
    dfs_btree_store_info_t & store_info) const {
    const mid_node_t * pmid_node = NULL;
    uint64_t slcnode_index = UNDEF_INDEX;
    uint32_t slcnode_num = 0;
    uint32_t slcnode_pos = 0;
    int log_level = DF_UL_LOG_NONE;
    int err = 0;
    ...
    } else if ((err = _store_slckeys(slctree_root, store_info)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "_store_slckeys() returns 0x%x", err);

    } else if (_is_mid_node(slctree_root)) {
        if ((err = _get_mid_node(slctree_root, &pmid_node)) != 0 || NULL == pmid_node) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_get_mid_node() returns 0x%x", err);
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_NODE_NULL_POINTER);
        } else if (pmid_node->get_slckey_num() <= 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pmid_node->get_slckey_num() <= 0");
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_NUM);
        } else {
            slcnode_num = pmid_node->get_slckey_num()+1;
            for (slcnode_pos = 0; slcnode_pos < slcnode_num; ++slcnode_pos) {
                if ((slcnode_index = pmid_node->get_slcnode_index(slcnode_pos)) == UNDEF_INDEX) {
                    log_level = DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level, "slcnode_index == UNDEF_INDEX");
                    err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCNODE_INDEX);
                    break;
                } else if ((err = _store_slctree(slcnode_index, store_info)) != 0) {
                    log_level = (ERRNO_BT_CKP_CANCELLED == err) ? DF_UL_LOG_NOTICE : DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level, "_store_slctree() returns 0x%x", err);
                    break;
                }
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        ...
                        slctree_root, slcnode_index);
        DF_WRITE_LOG_US(log_level,
                        "slcnode_num=%d, slcnode_pos=%d",
                        slcnode_num, slcnode_pos);
        if (NULL != pmid_node) {
            DF_WRITE_LOG_US(log_level,
                            "pmid_node->get_slckey_num()=%d",
                            pmid_node->get_slckey_num());
        }
    }
...
//load all contents of a slctree to file
int dfs_btree_t::_load_slctree(
    uint64_t & slctree_root,
    dfs_btree_load_info_t & load_info) {
    uint64_t slckey_index_ary[BT_FANOUT-1];
    uint64_t slcnode_index_ary[BT_FANOUT];
    uint64_t ref_counter = UNDEF_REF_COUNTER;
    uint32_t slckey_num = 0;
    uint32_t slcnode_pos = 0;
    uint32_t j = 0;
    ...
    if (_bt_is_cancel_checkpointing()) {
        ...
        err = DF_BT_SET_NORMAL_ERR(ERRNO_BT_CKP_CANCELLED);
    } else if ((err = _load_slckeys(
                          slckey_index_ary,
                          df_len_of_ary(slckey_index_ary),
                          slckey_num,
                          is_mid_node,
                          load_info)) != 0) {
        log_level = DF_UL_LOG_FATAL;
        DF_WRITE_LOG_US(log_level, "_load_slckeys() returns 0x%x", err);
    } else if (is_mid_node) {
        for (slcnode_pos = 0; slcnode_pos <= slckey_num; ++slcnode_pos) {
            if ((err = _load_slctree(
                           slcnode_index_ary[slcnode_pos],
                           load_info)) != 0) {
                log_level = (ERRNO_BT_CKP_CANCELLED == err) ? DF_UL_LOG_NOTICE : DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_load_slctree() returns 0x%x", err);
                ...
                if (load_info.get_store_t_type() != dfs_btree_load_info_t::STORE_T_INDEX) {
                    ...
                    if ((err1 = vir_inc_obj_ary_ref_counter(slckey_index_ary, slckey_num)) != 0) {
                        log_level = DF_UL_LOG_FATAL;
                        DF_WRITE_LOG_US(log_level, "vir_inc_obj_ary_ref_counter() returns 0x%x", err1);
                    } else if ((err1 = vir_dec_obj_ary_ref_counter(slckey_index_ary, slckey_num)) != 0) {
                        log_level = DF_UL_LOG_FATAL;
                        DF_WRITE_LOG_US(log_level, "vir_dec_obj_ary_ref_counter() returns 0x%x", err1);
                    }
                }
                //已经生成的子树也需要回收
                for (j = 0; j < slcnode_pos; ++j) {
                    //先+1引用计数，再-1引用计数，这导致对象回收
                    if ((err1 = _inc_node_ref_counter(slcnode_index_ary[j], ref_counter)) != 0) {
                        log_level = DF_UL_LOG_FATAL;
                        DF_WRITE_LOG_US(log_level, "_inc_node_ref_counter() returns 0x%x", err1);
                    } else if ((err1 = _dec_node_ref_counter(slcnode_index_ary[j], ref_counter)) != 0) {
                        log_level = DF_UL_LOG_FATAL;
        ...
        if (0 == err) {
            mid_node_t * pmid_node = NULL;
            if ((err = _acquire_mid_node(slctree_root, &pmid_node)) != 0) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_acquire_mid_node(slctree_root...) returns 0x%x", err);
            } else if (UNDEF_INDEX == slctree_root || NULL == pmid_node) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "UNDEF_INDEX == slctree_root || NULL == pmid_node");
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
            ...
            } else if ((err = _inc_pointed_ref_counter(slctree_root)) != 0) { //由该新节点导致的引用计数增加
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_inc_pointed_ref_counter(slctree_root) returns 0x%x", err);
            }
        }
        ...
        if ((err = _acquire_leaf_node(slctree_root, &pleaf_node)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_acquire_leaf_node(slctree_root...) returns 0x%x", err);
        } else if (UNDEF_INDEX == slctree_root || NULL == pleaf_node) {
            //leaf_node: 保留unit 0
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "UNDEF_INDEX == slctree_root || NULL == pleaf_node");
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_LEAF_NODE_ACQUIRE);
        } else if ((err = pleaf_node->put_slckeys(slckey_index_ary, slckey_num)) != 0) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "pleaf_node->put_slckeys() returns 0x%x", err);
        } else if ((err = _inc_pointed_ref_counter(slctree_root)) != 0) { //由该新节点导致的引用计数增加
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "_inc_pointed_ref_counter(slctree_root) returns 0x%x", err);
        }
    ...
    if (DF_UL_LOG_NONE != log_level) {
        DF_WRITE_LOG_US(log_level,
                        ...
                        slctree_root);
        DF_WRITE_LOG_US(log_level,
                        "slckey_num=%d, slcnode_pos=%d, is_mid_node=%d",
                        slckey_num, slcnode_pos, is_mid_node);
        BT_DF_WRITE_LOG_INT64S(log_level, "slckey_index_ary", slckey_index_ary, slckey_num);
        BT_DF_WRITE_LOG_INT64S(log_level, "slcnode_index_ary", slcnode_index_ary, slckey_num+1);
    }
...
//store contents of all slckeys of a node to file
int dfs_btree_t::_store_slckeys(
    const uint64_t node_index,
    dfs_btree_store_info_t & store_info) const {
    uint64_t num = 0;
    ...
    if ((err = _get_node(node_index, &pnode)) != 0 || NULL == pnode) {
        ...
        slckey_num = pnode->get_slckey_num();
        num = slckey_num<<1;
        store_info.add_total_key_num(slckey_num);
        ...
        if ((err = store_info.encode_num(num)) != 0) {
            ...
            for (slckey_pos = 0; slckey_pos < slckey_num; ++slckey_pos) {
                if ((obj_index = pnode->get_slckey_index(slckey_pos)) == UNDEF_INDEX) {
                    log_level = DF_UL_LOG_FATAL;
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        ...
                        pre_data_pos, store_info.get_data_pos());
        DF_WRITE_LOG_US(log_level,
                        "slckey_num=%d, slckey_pos=%d, num=%ld",
                        slckey_num, slckey_pos, num);
    }
...
int dfs_btree_t::_load_slckeys(
    ...
    if ((err = load_info.decode_num(num)) != 0) {
        ...
        //slckey_num是0是可能的：空的树
        if ((slckey_num = (uint32_t)(num>>1)) > ary_len) {
            log_level = DF_UL_LOG_FATAL;
            DF_WRITE_LOG_US(log_level, "slckey_num > ary_len");
            err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_SLCKEY_NUM);
        }
        load_info.slc_total_key_num(slckey_num);
    }
    ...
    if (0 == err) {
        for (slckey_pos = 0; slckey_pos < slckey_num; ++slckey_pos) {
            ...
            if (load_info.get_store_t_type() == dfs_btree_load_info_t::STORE_T_INDEX) {
                if ((err = load_info.get_kept_key_index(obj_index, slckey_index_ary[slckey_pos])) != 0) {
                    log_level = DF_UL_LOG_FATAL;
                    DF_WRITE_LOG_US(log_level, "load_info.get_kept_key_index() returns 0x%x", err);
                    break;
                }
            } else {
                slckey_index_ary[slckey_pos] = obj_index;
                if (load_info.get_store_t_type() ==
                        dfs_btree_load_info_t::STORE_T_VALUE_KEEP_T_INDEX) {
                    if ((err = load_info.keep_obj_index(slckey_index_ary[slckey_pos])) != 0) {
                        log_level = DF_UL_LOG_FATAL;
    ...
    if (DF_UL_LOG_NONE != log_level) {
        ...
        DF_WRITE_LOG_US(log_level,
                        "slckey_num=%d, slckey_pos=%d, ary_len=%d, is_mid_node=%d",
                        slckey_num, slckey_pos, ary_len, is_mid_node);
        BT_DF_WRITE_LOG_INT64S(log_level, "slckey_index_ary", slckey_index_ary, slckey_num);
    }
