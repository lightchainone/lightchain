...
class _Lsc_kv_btree_build_iterator_base {
plclic:
    ...
    virtual ~_Lsc_kv_btree_build_iterator_base() {
    }
};
...
class _Lsc_kv_btree_build_iterator {
plclic:
    //相关的迭代器..
    ...
    typedef _Lsc_kv_btree_build_iterator<DU,
            kreference,kpointer,
            reference,pointer,
    ...
    //自己是self_type..
    typedef _Lsc_kv_btree_build_iterator self_type;
    //操纵的btree.
    typedef _Lsc_kv_btree_build_iterator_base<DU> kv_btree_type;

private:
    friend class _Lsc_kv_btree_build<DU,ROW_SIZE,BTreeAdapter>;
...
plclic:
    _Lsc_kv_btree_build_iterator()
        :_btree(NULL),_du(),_end(true) {
        //nothing.
    }
    _Lsc_kv_btree_build_iterator(const kv_btree_type *btree,
                                 const DU &du,
                                 bool end=false):_btree(btree),_du(du),_end(end) {
    }
    _Lsc_kv_btree_build_iterator(const iterator & iter):
        _btree(iter._btree),_du(iter._du),_end(iter._end) {
    }
    _Lsc_kv_btree_build_iterator(const const_iterator & iter):
        _btree(iter._btree),_du(iter._du),_end(iter._end) {
    }
...
template<typename DU,
         ...
         unsigned int ROW_SIZE,
         typename BTreeAdapter>
class _Lsc_kv_btree_build_reverse_iterator {
plclic:
    //相关的迭代器..
    typedef typename DU::key_type key_type;
    typedef typename DU::value_type value_type;
    ...
    typedef const value_type * cpointer;
    typedef const value_type & creference;
    typedef _Lsc_kv_btree_build_reverse_iterator<DU,
            kreference,kpointer,
            reference,pointer,
    ...
    //自己是self_type
    typedef _Lsc_kv_btree_build_reverse_iterator self_type;
    //自己操纵的btree
    typedef _Lsc_kv_btree_build_iterator_base<DU> kv_btree_type;
...
plclic:
    _Lsc_kv_btree_build_reverse_iterator()
        :_btree(NULL),_du(),_end(true) {
        //nothing.
    }
    _Lsc_kv_btree_build_reverse_iterator(const kv_btree_type *btree,
                                         const DU &du,
                                         bool end=false):
        _btree(btree),_du(du),_end(end) {
    }
    _Lsc_kv_btree_build_reverse_iterator(const reverse_iterator & iter):
        _btree(iter._btree),_du(iter._du),_end(iter._end) {
    }
    _Lsc_kv_btree_build_reverse_iterator(const const_reverse_iterator & iter):
        _btree(iter._btree),_du(iter._du),_end(iter._end) {
    }
    //如果用户在这里已修改iterator的话
    //是一个安全的操作,不会反映到btree
...
//============================================================
template<typename DU>
class _Lsc_kv_btree_search_iterator_base {
plclic:
    virtual bool get_smallest(DU **du)const=0;
    virtual bool get_largest(DU **du)const=0;
    ...
    virtual bool get_larger(const DU &pdu,DU **du,
                            dfs_btree_drill_t *pdrill_info)const=0;
    virtual ~_Lsc_kv_btree_search_iterator_base() {
    }
};
template<typename DU,
         typename KRef,
         ...
         unsigned int ROW_SIZE,
         typename BTreeAdapter>
class _Lsc_kv_btree_search_iterator {
plclic:
    //相关的迭代器..
    typedef typename DU::key_type key_type;
    typedef typename DU::value_type value_type;
    ...
    typedef const value_type * cpointer;
    typedef const value_type & creference;
    typedef _Lsc_kv_btree_search_iterator<DU,
            kreference,kpointer,
            reference,pointer,
    ...
    typedef _Lsc_kv_btree_search_iterator<DU,
            ...
            const_iterator;
    //自己是self_type
    typedef _Lsc_kv_btree_search_iterator self_type;
    //操纵的btree.
    typedef _Lsc_kv_btree_search_iterator_base<DU> kv_btree_type;

private:
    friend class _Lsc_kv_btree_search<DU,ROW_SIZE,BTreeAdapter>;
    const kv_btree_type *_btree;
    //data.
    ...
    bool _end;
plclic:
    _Lsc_kv_btree_search_iterator()
        :_btree(NULL),_du(NULL),_drill(),_end(true) {
        //nothing.
    }
    _Lsc_kv_btree_search_iterator(const kv_btree_type *btree,
                                  DU *du,
                                  bool end=false)
        :_btree(btree),_du(du),_drill(),_end(end) {
    }
    _Lsc_kv_btree_search_iterator(const iterator & iter):
        _btree(iter._btree),_du(iter._du),_end(iter._end) {
        _drill=iter._drill;
    }
    _Lsc_kv_btree_search_iterator(const const_iterator & iter):
        _btree(iter._btree),_du(iter._du),_end(iter._end) {
        _drill=iter._drill;
    }
...
template<typename DU,
         ...
         unsigned int ROW_SIZE,
         typename BTreeAdapter>
class _Lsc_kv_btree_search_reverse_iterator {
plclic:
    //相关的迭代器..
    typedef typename DU::key_type key_type;
    typedef typename DU::value_type value_type;
    ...
    typedef const value_type * cpointer;
    typedef const value_type & creference;
    typedef _Lsc_kv_btree_search_reverse_iterator<DU,
            kreference,kpointer,
            reference,pointer,
            ROW_SIZE,BTreeAdapter>
            reverse_iterator;
    ...
    //自己是self_type
    typedef _Lsc_kv_btree_search_reverse_iterator self_type;
    //操纵的btree.
    typedef _Lsc_kv_btree_search_iterator_base<DU> kv_btree_type;
...
plclic:
    _Lsc_kv_btree_search_reverse_iterator()
        :_btree(NULL),_du(NULL),_drill(),_end(true) {
        //nothing.
    }
    _Lsc_kv_btree_search_reverse_iterator(const kv_btree_type *btree,
                                          DU *du,
                                          bool end=false):
        _btree(btree),_du(du),_drill(),_end(end) {
    }
    _Lsc_kv_btree_search_reverse_iterator(const reverse_iterator & iter):
        _btree(iter._btree),_du(iter._du),_end(iter._end) {
        _drill=iter._drill;
    }
    _Lsc_kv_btree_search_reverse_iterator(const const_reverse_iterator & iter):
        _btree(iter._btree),_du(iter._du),_end(iter._end) {
        _drill=iter._drill;
    }
