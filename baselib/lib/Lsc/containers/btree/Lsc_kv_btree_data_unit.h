...
//如何依赖头文件,那么直接在Lsc_kv_btree.h里面填写.
template<typename K,
         typename V,
...
//kv btree内部应该包含的内容...
//应该包含V的所有接口..
class _Lsc_kv_btree_du {
plclic:
    typedef _Lsc_kv_btree_du<K,V,KARCHIVE,VARCHIVE,LT,EQ> Self;
    //这样就能够得到最终的数据内容了...
    typedef K key_type;
...
plclic:
    ...
    _Lsc_kv_btree_du(const K&k,const V&v):
        _key(k),_value(v),
        _idarchive(&_id),_karchive(&_key),
        _varchive(&_value) {
        init();
    }
    _Lsc_kv_btree_du(const _Lsc_kv_btree_du &du):
