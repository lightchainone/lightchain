...
class dfs_xz_btree_fhead_t : plclic dfs_btree_fhead_base_t, plclic dfs_xz_btree_fhead_ext_t {
};
...
protected:
    ...
    inline IXINTUI_INLINE int _get_exist_obj(const uint64_t obj_index, PCDU & pcdu) const ;
    //return: 0 for success, other values for error
...
private:
    ...
    inline IXINTUI_INLINE int _compare_obj_obj(const DU & duobj1, const DU & duobj2, int & cmp) const;
    inline IXINTUI_INLINE int _compare_obj_key(const DU & duobj1, const K & key2, int & cmp) const;
    inline IXINTUI_INLINE int _inc_obj_ref_counter(const uint64_t obj_index, uint64_t & ref_counter);
    inline IXINTUI_INLINE int _dec_obj_ref_counter(const uint64_t obj_index, uint64_t & ref_counter);
protected:
    ////  return: 0 for success, other values for error
...
//Notice: the obj must exist.
template<typename T, typename K, uint32_t ROW_SIZE>
inline IXINTUI_INLINE int _dfs_xz_btree_t<T, K, ROW_SIZE>::_get_exist_obj(
    const uint64_t obj_index,
    PCDU & pcdu) const {
    PDU pdu = NULL;
    int log_level = DF_UL_LOG_NONE;
...
//cmp: -1, 0, 1 for less than, equal, great than
template<typename T, typename K, uint32_t ROW_SIZE>
inline IXINTUI_INLINE int _dfs_xz_btree_t<T, K, ROW_SIZE>::_compare_obj_key(
    const DU & duobj1,
    const K & key2,
    int & cmp) const {
    if (duobj1 == key2) {
    ...
    if ((err = _get_exist_obj(obj2_index, pcdu2)) != 0 || NULL == pcdu2) {
        ...
        if (0 == err && 0 == cmp) {
            if (pos1 >= obj_num || pos1 < 0) {
                ...
                err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_SLCKEY_OVERFLOW);
            } else {
...
int _dfs_xz_btree_t<T, K, ROW_SIZE>::vir_search_ary_by_key(
    ...
    if (NULL == pckey2) {
        ...
        while (pos1 <= pos2) {
            mid = (pos1+pos2)/2;
            if (IXINTUI_UNLIKELY((err = _get_exist_obj(obj_index_ary[mid], pcdu1)) != 0 || NULL == pcdu1)) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_get_exist_obj(obj_index_ary[mid]...) returns 0x%x", err);
                err = DF_BT_SET_INTERNAL_ERR(ERRNO_BT_OBJ_NULL_POINTER);
                break;
            } else if (IXINTUI_UNLIKELY((err = _compare_obj_key(*pcdu1, *pckey2, cmp)) != 0)) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "_compare_obj_key() returns 0x%x", err);
                break;
            } else if (0 == cmp) {
        ...
        ins_pos = pos1;
        if (0 == err && 0 == cmp) {
            if (IXINTUI_UNLIKELY(pos1 >= obj_num || pos1 < 0)) {
                log_level = DF_UL_LOG_FATAL;
                DF_WRITE_LOG_US(log_level, "cmp == 0 && (pos1 >= obj_num || pos1 < 0)");
                err = DF_BT_SET_INTERNAL_ERR(this->ERRNO_BT_SLCKEY_OVERFLOW);
            } else {
...
bool dfs_xz_btree_t<T, K, ROW_SIZE>::search(
    ...
    } else if(IXINTUI_UNLIKELY(!_rc_root_set.is_valid_in_use_indexer(*prc_indexer))) {
        log_level = DF_UL_LOG_WARNING;
    ...
    if (0 == err) {
        ...
        if (IXINTUI_UNLIKELY((err = this->_search_by_key(*pbt_root,
                                  (const void *)&key,
                                  drill_info,
                                  obj_index)) != 0)) {
            log_level = DF_UL_LOG_FATAL;
