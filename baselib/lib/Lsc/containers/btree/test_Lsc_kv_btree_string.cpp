...
typedef Lsc::Lsc_kv_btree_build<Key_t,Data_t> kv_btree_build_t;
typedef Lsc::Lsc_kv_btree_search<Key_t,Data_t> kv_btree_search_t;
...
int test_insert(kv_btree_build_t &bt){
    for(int i=0; i<10; i++) {
        ...
        Data_t v=strfy(i);
        //插入成功..
        assert(bt.set(k,v)==Lsc::BTREE_INSERT_SUC);
    }
    for(int i=0; i<10; i++) {
        Key_t k=strfy(i);
        Data_t v=strfy(i);
        //数据存在..
        assert(bt.set(k,v)==Lsc::BTREE_EXIST);
    }
    for(int i=0; i<10; i++) {
        Key_t k=strfy(i);
        Data_t v=strfy(i);
        //数据被覆盖..
        assert(bt.set(k,v,!0)==Lsc::BTREE_OVERWRITE);
    }
    for(int i=10; i<20; i++) {
        Key_t k=strfy(i);
        Data_t v=strfy(i);
        assert(bt.set(k,v,!0)==Lsc::BTREE_INSERT_SUC);
    }
    return 0;
}
...
int test_get(kv_btree_build_t &bt){
    for(int i=0; i<20; i++) {
        Key_t k=strfy(i);
        Data_t v;
        assert(bt.get(k,&v)==Lsc::BTREE_EXIST);
        assert(bt.get(k)==Lsc::BTREE_EXIST);
        assert(v==k);
    }
    for(int i=20; i<30; i++) {
        Key_t k=strfy(i);
        Data_t v;
        assert(bt.get(k,&v)==Lsc::BTREE_NOEXIST);
    }
    return 0;
}
...
int test_erase(kv_btree_build_t &bt){
    for(int i=10; i<20; i++) {
        Key_t k=strfy(i);
        assert(bt.erase(k)==Lsc::BTREE_EXIST);
    }
    for(int i=20; i<30; i++) {
        Key_t k=strfy(i);
        assert(bt.erase(k)==Lsc::BTREE_NOEXIST);
    }
    return 0;
}
...
int test_store_checkpoint(kv_btree_build_t &bt){
    assert(bt.make_checkpoint()==Lsc::BTREE_OK);
    int fd=-1;
    fd=open("bt.img",O_WRONLY | O_CREAT,0666);
    assert(fd!=-1);
    assert(bt.store(fd,0)==Lsc::BTREE_OK);
    assert(bt.end_checkpoint()==Lsc::BTREE_OK);
    close(fd);
    return 0;
}
...
int test_load_checkpoint(kv_btree_build_t &bt){
    ...
    assert(fd!=-1);
    assert(bt.load(fd,0)==Lsc::BTREE_OK);
    close(fd);
    return 0;
}
...
int test_snapshot(kv_btree_build_t &bt,
    ...
    assert(bt.snapshot(sbt)==Lsc::BTREE_OK);
    return 0;
}
...
int test_store_snapshot(kv_btree_search_t &sbt){
    ...
    fd=open("bt.img",O_WRONLY | O_CREAT,0666);
    assert(fd!=-1);
    assert(sbt.store(fd,0)==Lsc::BTREE_OK);
    close(fd);
    return 0;
}
