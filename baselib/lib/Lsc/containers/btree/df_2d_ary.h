...
#define ERRINFO_BT_SLCKEY_POS               ""
#define ERRINFO_BT_SLCKEY_NUM               ""
#define ERRINFO_BT_SLCKEY_ORDER             ""
#define ERRINFO_BT_SLCKEY_OVERFLOW          ""
#define ERRINFO_BT_SLCKEY_UNDERFLOW         ""
#define ERRINFO_BT_SLCKEY_INDEX             ""
#define ERRINFO_BT_SLCNODE_INDEX            ""
...
plclic:
    ...
    enum DFS_BT_ERR_NO {
        ...
        ERRNO_BT_SLCKEY_POS                     ,
        ERRNO_BT_SLCKEY_NUM                     ,
        ERRNO_BT_SLCKEY_ORDER                   ,
        ERRNO_BT_SLCKEY_OVERFLOW                ,
        ERRNO_BT_SLCKEY_UNDERFLOW               ,
        ERRNO_BT_SLCKEY_INDEX                   ,
        ERRNO_BT_SLCNODE_INDEX                  ,
        ERRNO_BT_CKP_STATE                      ,
...
class dfs_2d_base_t : plclic dfs_bt_const_t {
private:
...
template<typename T>
class dfs_2d_du_t : plclic dfs_2d_base_t, plclic T {
private:
    typedef T *PT;
    typedef const T *PCT;
plclic:
...
class dfs_u_int64_t : plclic dfs_bt_const_t {
private:
    uint64_t _data;
plclic:
...
class dfs_id_t : plclic dfs_bt_const_t {
private:
    uint64_t _id;
plclic:
    dfs_id_t() {
...
class dfs_id_data_t : plclic dfs_id_t {
private:
    uint64_t _data;
plclic:
...
class dfs_s_ary_t : virtual plclic dfs_bt_const_t {
private:
    typedef T *PT;
    typedef const T *PCT;
private:
...
class dfs_rc_set_base_t : virtual plclic dfs_bt_const_t {
...
class dfs_rc_indexer_t : plclic dfs_bt_const_t {
...
class dfs_rc_indexer_ext_t : plclic dfs_rc_indexer_t {
plclic:
...
class dfs_rc_set_t : plclic dfs_rc_set_base_t {
private:
    typedef T *PT;
    typedef const T *PCT;
...
private:
    inline IXINTUI_INLINE int _get_exist_du(const uint64_t t_index, PDU & pdu) const {
...
plclic:
    ...
    //说明：需要保证获得的项没有被回收
    inline IXINTUI_INLINE int retrieve_cur_index(dfs_rc_indexer_t & rc_indexer) const {
...
//          规则：plclic接口仅提供“只读”操作，protected接口提供修改操作。
//
...
class dfs_bt2d_ary_t : virtual plclic dfs_bt_const_t {
private:
    typedef T *PT;
    typedef const T *PCT;
    typedef dfs_2d_du_t<T> DU;
...
plclic:
    //Return: 0 for success, other values for error
    inline IXINTUI_INLINE int get_exist_t_unit(const uint64_t t_index, PT & pt) const {
        PDU pdu = NULL;
        int log_level = DF_UL_LOG_NONE;
        int err = 0;
...
plclic:
    inline IXINTUI_INLINE bool is_valid_t_index(const uint64_t t_index) const {
        return (t_index < _t_index_for_next_row);
    };
    int valid_t_index_verify(const uint64_t t_index) const {
        uint64_t true_t_index = t_index;
