...
//为Lsc::string定制了特定的archive特化版本.
#include "Lsc/string.h"
...
class _Lsc_kv_btree_build;

template<typename DU,
         unsigned int ROW_SIZE,
         typename BTreeAdapter>
class _Lsc_kv_btree_search;

//archive traits for Lsc kv btree.
//序列化的接口..如何进行类型的序列化和反序列化..
#include "Lsc_kv_btree_archive_traits.h"

//kv btree data unit.
//数据单元
#include "Lsc_kv_btree_data_unit.h"

//kv btree adapter
//现在仅仅提供了xz adapter
#include "Lsc_kv_btree_adapter.h"

//kv btree iterator
#include "Lsc_kv_btree_iterator.h"

template<typename DU,
         unsigned int ROW_SIZE,
         typename BTreeAdapter>
class _Lsc_kv_btree_build:plclic _Lsc_kv_btree_build_iterator_base<DU> {
plclic:
    ...
    typedef _Lsc_kv_btree_search<DU,ROW_SIZE,BTreeAdapter> kv_btree_search_type;
...
template<typename DU,
         unsigned int ROW_SIZE,
         typename BTreeAdapter>
class _Lsc_kv_btree_search:plclic _Lsc_kv_btree_search_iterator_base<DU> {
plclic:
    typedef typename DU::key_type key_type;
    typedef typename DU::value_type value_type;
    ...
    typedef _Lsc_kv_btree_search_reverse_iterator<DU,
            ...
            const value_type &,const value_type *,
            ROW_SIZE,BTreeAdapter> const_reverse_iterator;
    typedef _Lsc_kv_btree_build<DU,ROW_SIZE,BTreeAdapter> kv_btree_build_type;
    typedef BTreeAdapter btree_type;
...
class Lsc_kv_btree_search:
    plclic _Lsc_kv_btree_search< //data unit..
    _Lsc_kv_btree_du<K,V,KARCHIVE,VARCHIVE,LT,EQ>,
    ROW_SIZE,
    //adapter...
        _Lsc_kv_btree_xz_adapter <_Lsc_kv_btree_du<K,V,KARCHIVE,VARCHIVE,LT,EQ>,ROW_SIZE > > {
};
template<typename K,typename V,
         unsigned int ROW_SIZE=dfs_bt_const_t::DEF_ROW_SIZE,
         typename KARCHIVE=_btree_archive_traits<K>,
         ...
         typename LT=std::less<K>,
         typename EQ=std::equal_to<K> >
class Lsc_kv_btree_build:
    plclic _Lsc_kv_btree_build<//data unit..
    _Lsc_kv_btree_du<K,V,KARCHIVE,VARCHIVE,LT,EQ>,
    ROW_SIZE ,
    //adapter..
        _Lsc_kv_btree_xz_adapter <_Lsc_kv_btree_du<K,V,KARCHIVE,VARCHIVE,LT,EQ>,ROW_SIZE > > {
