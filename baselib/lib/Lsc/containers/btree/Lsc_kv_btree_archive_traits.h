...
//如果依赖头文件,那么直接在Lsc_kv_btree.h里面填写.
//btree的archive接口...汗...
template<typename T>
...
//Lsc::string和std::string应该是走同样的逻辑的..
template<>
class _btree_archive_traits<Lsc::string> {
    Lsc::string *_ptr;

plclic:
    _btree_archive_traits(Lsc::string *ptr):_ptr(ptr) {
    }
    _btree_archive_traits():_ptr(NULL) {
    }
    uint64_t size()const {
    ...
    int deserialize(char *buf,const uint64_t buf_size,
        ...
        //Lsc construct没有使用两个参数初始化的...
        //所以是用原始的..
        ::new(static_cast<void*>(_ptr))Lsc::string(str,siz);
        return BTREE_OK;
    }
};
