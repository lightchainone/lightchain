...
int int_string_change(const ConfigUnit &keyold, const ConfigUnit &keynew, void * ) {
	printf("old:%s, new:%s\n", keyold.to_Lsc_string().c_str(), keynew.to_Lsc_string().c_str());
	if (g_run) {
		if (++g_run >3) {
			g_run = 0;
		}
...
int float_key_change(const ConfigUnit &keyold, const ConfigUnit &keynew, void * ) {
	printf("[%s]old:%f, new:%f\n",keynew.get_key_name().c_str(), keyold.to_dolcle(), keynew.to_dolcle());
	if (g_run) {
		if (++g_run >3) {
			g_run = 0;
		}
...
int main(){
	...
	printf("add char:%c\n", rd.get_config()->deepGet("ivar.a").to_char());
	rd.add_key_string("lcclient.proto[4].method[5].name", int_string_change, NULL);
	printf("add string:%s\n", rd.get_config()->deepGet("lcclient.proto[4].method[5].name").to_Lsc_string().c_str());
